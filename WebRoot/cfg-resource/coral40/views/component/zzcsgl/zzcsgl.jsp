  ...
  <div class="fill">
    <div class="toolbarsnav clearfix">
      <ces:toolbar id="toolbarId${idSuffix}" onClick="$.ns('namespaceId${idSuffix}').toolbarClick"
                   data="['->',{'label': '打印', 'id':'dy', 'disabled': 'false','type': 'button'},{'label': '保存', 'id':'save', 'cls':'save_tb','disabled': 'false','type': 'button'},{'label': '关闭', 'id':'CFG_closeComponentZone','cls':'return_tb', 'disabled': 'false','type': 'button'}]">
      </ces:toolbar>
      <div class='homeSpan' style="margin-top: -23px;"><div><div style='margin-left:20px;width: 150px;' id="nva${idSuffix}"> - 分拣管理 - 新增 </div></div></div>
    </div>
    <form id="csForm${idSuffix}" action="zzcsgl!saveCsxx.json"
      ...
      <div class="fillwidth colspan3 clearfix">
        ...
        <div class="app-inputdiv4">
          ...
          <input id="ZZDYBH${idSuffix}" name="ZZDYBH" data-options="" />
        ...
        <div class="app-inputdiv4">
          ...
          <input id="PL${idSuffix}" name="PL" readonly="readonly" class="coralui-textbox"/>
        </div>
        <!------------------ 第二排结束---------------->
        ...
        <div class="app-inputdiv4">
          ...
          <input id="PZ${idSuffix}" name="PZ" readonly="readonly"  class="coralui-textbox"/>
        </div>
    ...
    <ces:grid id="gridId${idSuffix}" afterInlineSaveRow="afterInlineSaveRow"
      ...
      <ces:gridCols>
        ...
        <ces:gridCol name="PCH"  editable="false" align="center" edittype="text" editoptions="required: false" width="180">采收批次号</ces:gridCol>
        <ces:gridCol name="ZL" formatter="text" align="center" formatoptions="required: false,validType:'naturalnumber'" width="80">重量(KG)</ces:gridCol>
        <ces:gridCol name="KCZL" hidden="true"  formatoptions="required: false" width="180">库存重量(KG)</ces:gridCol>
        <ces:gridCol name="DYZS" formatter="text" align="center" formatoptions="required: false,validType:'naturalnumber'" width="80">打印张数</ces:gridCol>
...
<script type="text/javascript">
  ...
  $.extend($.ns("namespaceId${idSuffix}"), {
    ...
    toolbarClick : function(event, button) {
      ...
      } else if(button.id == "dy"){

        <%--var pid=$("#ID${idSuffix}").textbox("getValue");--%>
        if($('#ID${idSuffix}').val()==''){
          CFG_message("打印前保存!","error");
          return;
        }
        var gridDate=$('#gridId${idSuffix}').grid('getRowData');
        if(isSwt || _print){

          var obj = $.loadJson($.contextPath + "/trace!getQybmAndQymc.json?sysName=ZZ");
          var qymc = obj.qymc;
          var qvmc=$("#QYBH${idSuffix}").combobox('getText');
          var dkmc=$("#DKBH${idSuffix}").combobox('getText');
          var pz=$("#PZ${idSuffix}").textbox('getValue');
          var cssj=$("#CSSJ${idSuffix}").datepicker('getDateValue');
          for(var numi=0;numi<4;numi++){
            var printtimes;
            var pch1=$("#gridId${idSuffix}").grid("getRowData",numi).PCH;

              var gridDate=$('#gridId${idSuffix}').grid('getRowData');
              if(($("#gridId${idSuffix}").grid("getRowData",numi+1).DYZS!='')&&($("#gridId${idSuffix}").grid("getRowData",numi+1).DYZS!=undefined)){
                printtimes=parseInt($("#gridId${idSuffix}").grid("getRowData",numi+1).DYZS),
                        printdata={
                          scqy:qymc,
                          qy:$("#QYBH${idSuffix}").combobox('getText'),
                          dk:$("#DKBH${idSuffix}").combobox('getText'),
                          pz:$("#PZ${idSuffix}").val(),
                          cssj:cssj,
                          cspch:$("#gridId${idSuffix}").grid("getRowData",numi+1).PCH,
                        };
                if(isSwt){
                  ...
                  for(var times=0;times<printtimes;times++){
                    var result=_print.print("fjglPrint",{
                      "SCQY":"生产企业 ：" + printdata.scqy,
                      "QY":"        区域 ：" + printdata.qy,
                      "DK":"        地块 ：" + printdata.dk,
                      "PZ":"        品种 ：" + printdata.pz,
                      "CSSJ":"采收时间 ：" + printdata.cssj,
                      "PCH": printdata.cspch,
                    });
                  }
                }
  ...
  $(function() {
    var configInfo = CFG_initConfigInfo({
      ...
      'bodyOnLoad' : function(configInfo) {
        $('#CSSJ${idSuffix}').datepicker('setDate',new Date());
        ...
        $("#DKBH${idSuffix}").combobox('option','onChange',function(e,data){
          ...
          var scdabhData = $.loadJson($.contextPath + "/zzcsgl!getScdabhByDkbh.json?DKBH="+dkbh);
        ...
        $("#ZZDYBH${idSuffix}").combobox('option','onChange',function(e,data){
          ...
          $("#SCDABH${idSuffix}").combobox('setValue','');
          var jsonData=$.loadJson($.contextPath + "/zzcsgl!getScdabh.json?ZZDYBH="+zzdymc);
          $("#SCDABH${idSuffix}").combobox("reload",jsonData);
          $("#SCDABH${idSuffix}").combobox('enable');
          $("#ZZDYMC${idSuffix}").textbox('setValue',$("#ZZDYBH${idSuffix}").combobox('getText'));
          document.getElementById('PL${idSuffix}').value='';
          document.getElementById('PZ${idSuffix}').value='';
          //document.getElementById('ZZDYMC${idSuffix}').value=data.value;
        });
        ...
        if(isEmpty(inputParamId)){//如果是新增操作在设置临时ID
          ...
          for(var i=0;i<$zldj.length;i++){
            ...
            $grid.grid("setCell", lastsel,'PCH',cslsh+'0'+(i+1));
            $grid.grid("setCell",lastsel,'PID',$('#ID${idSuffix}').textbox('getValue'));
          }
        }else{//修改操作初始化页面操作
          $("#nva${idSuffix}").html(" - 分拣管理 - 修改");
  ...
  $("#csForm${idSuffix}").submit(function(){
    ...
    if($('#ID${idSuffix}').val()==''){
      ...
      //修改时判断如果重量和库存重量不同,不让修改
      for(var k=0;k<rownumber;k++){
//        if(($grid.grid("getRowData","tem_"+k).ZL+'').length>11){
//          CFG_message("采收重量过大！", "error");return false;
//        }
        if((($grid.grid("getRowData","tem_"+k).DYZS!='')&&($grid.grid("getRowData","tem_"+k).ZL==''))){
          CFG_message("没有重量时不可填打印张数！", "error");return false;
        }
...
//          CFG_message("采收重量过大！", "error");return false;
//        }
        if(($grid.grid("getRowData","tem_"+k).DYZS>100000000)){
          dyflag=false;
        }

      };
      if(!dyflag){
        CFG_message("打印张数太大！", "error");
        return false;
      }
      //判断是否有采收重量
      var flag1=true
      for(var k=0;k<rownumber;k++){
        if(($grid.grid("getRowData","tem_"+k).ZL!='')){
          flag1=false;
        }
      };
      if(flag1){ CFG_message("采收没重量不可保存！", "error");return false;}
//判断是否下面列表全为空
//      var flag=0;
//      for(var k=0;k<rownumber;k++){
//        if(($grid.grid("getRowData","tem_"+k).ZL!='')){flag++;}
//      };
//      if(flag==0){CFG_message("必须填写一行！", "error");return false;}
      ...
      for(var i=0;i<rownumber;i++){
        if($("#gridId${idSuffix}").grid("getRowData","tem_"+i).ZL!=''){
          ...
          zlhj1=parseInt(zlhj1)+zl;}
      }
      //获得1到3等品重量值和
      for(var i=0;i<rownumber-1;i++){
        if($("#gridId${idSuffix}").grid("getRowData","tem_"+i).ZL!=''){
          var zl=parseInt($("#gridId${idSuffix}").grid("getRowData","tem_"+i).ZL);
          kczl1=parseInt(kczl1)+zl;}
      }

      //重量合计和库存重量赋值
      $("#ZLHJ${idSuffix}").textbox('setValue',zlhj1);
      $("#KCZL${idSuffix}").textbox('setValue',kczl1);
...
//判断列表中是否重量和打印张数是否同时填写
    //($grid.grid("getRowData","tem_"+0).DYZS!='')&&($grid.grid("getRowData","tem_"+0).ZL=='')
    for(var k=1;k<5;k++){
      if((($grid.grid("getRowData",k).DYZS!='')&&($grid.grid("getRowData",k).ZL==''))){
        CFG_message("没有重量时不可填打印张数！", "error");return false;
      };


      //判断该条记录是否已被出场,如果出场不能修改
      var onegriddata=$.loadJson($.contextPath + "/zzcsgl!getOneDataforxG.json?id="+$grid.grid("getRowData",k).ID);

      if((onegriddata.ZL!=$grid.grid("getRowData",k).ZL)&&(onegriddata.KCZL!=onegriddata.ZL)){
        CFG_message("第"+k+"列已经出场,不能修改", "error");return false;
      };

      $grid.grid("getRowData",k).KCZL=$grid.grid("getRowData",k).ZL;
      //getCellComponent
      if(onegriddata.ZL!=$grid.grid("getRowData",k).ZL){
        //$grid.grid("getCellComponent",k,"KCZL").textbox('setValue',$grid.grid("getRowData",k).ZL);
        $grid.grid("setCell", k,'KCZL',$grid.grid("getRowData",k).ZL);
        if(k!=4){xgzl=xgzl+$grid.grid("getRowData",k).ZL-onegriddata.ZL;}
...
//    var flag=0;
//    for(var k=1;k<5;k++){
//      if(($grid.grid("getRowData",k).DYZS!='')&&($grid.grid("getRowData",k).ZL!='')){flag++;}
//    };
//    if(flag==0){CFG_message("必须填写一行！", "error");return false;}
    var editrow = $grid.grid("option", "editrow");
    var formdata = new FormData(this);
      ...
      $.ajax({
        ...
        success: function (data) {
         ...
         // $("#CSLSH${idSuffix}").textbox("setValue", data.CSLSH);
          ...
          for(var m=1;m<5;m++){
            griddata=$grid.grid("getRowData",m)

              $.ajax({
                url:$.contextPath+"/zzcsgl!saveNzwxq.json?qybm="+qybm,
                type:"post",
                data:$grid.grid("getRowData",m),
                dataType:"json",
//                async: false,
                success:function(res){
                  var pid=$("#ID${idSuffix}").textbox("getValue");
...
//                  $grid.grid("reload");
          ...
          for(var i=1;i<5;i++){
            if($("#gridId${idSuffix}").grid("getRowData",i).ZL!=''){
              var dawdaw=$("#gridId${idSuffix}").grid("getRowData",i);
              var zdad=$("#gridId${idSuffix}").grid("getRowData",i).ZL;
              var zl=parseInt($("#gridId${idSuffix}").grid("getRowData",i).ZL);
              zlhj=parseInt(zlhj)+zl;}
          }
          //获得1到3等品重量值和
          for(var i=1;i<4;i++){
          ...
          //将库存重量和采收重量同步到采收表中
          $.ajax({
            url:$.contextPath+"/zzcsgl!updateCsglZl.json?id="+ $("#ID${idSuffix}").textbox("getValue")+"&kczl="+xgzl+"&zlhj="+zlhj,
            type:"post",
            //data:$grid.grid("getRowData","tem_"+m),
            dataType:"json",
            success:function(res){
