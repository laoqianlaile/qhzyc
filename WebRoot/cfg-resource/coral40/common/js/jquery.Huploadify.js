(function($){
	$.fn.Huploadify = function(opts){
		var itemTemp = '<div id="${fileID}" class="h-uploadify-queue-item"><div class="h-uploadify-progress"><div class="h-uploadify-progress-bar"></div></div><span class="h-up_filename" title="${fileName}">${fileName}</span><a href="javascript:void(0);" class="h-uploadbtn">上传</a><a href="javascript:void(0);" class="h-delfilebtn">删除</a></div>';
		var defaults = {
		...
		this.each(function(index, element){
			...
			var uploadManager = {
				...
				init : function(){
					...
					inputStr += '"/>';
					inputStr += '<a id="file_upload_'+instanceNumber+'-button" href="javascript:void(0)" class="h-uploadify-button">';
					inputStr += option.buttonText;
					inputStr += '</a>';
					var uploadFileListStr = '<div id="file_upload_'+instanceNumber+'-queue" class="h-uploadify-queue"></div>';
					...
					returnObj =  {
						...
						disable : function(instanceID){
							...
							parent.find('.h-uploadify-button').css('background-color','#888').off('click');
						},
						ennable : function(instanceID){
							//点击上传按钮时触发file的click事件
							var parent = instanceID ? $('file_upload_'+instanceID+'-button') : $('body');
						  	parent.find('.h-uploadify-button').css('background-color','#707070').on('click',function(){
					...
					_this.find('.h-uploadify-button').on('click',function(){
				...
				_getFileList : function(){
					return _this.find('.h-uploadify-queue');
				},
				//根据选择的文件，渲染DOM节点
				_renderFile : function(file){
					...
					//如果是非自动上传，显示上传按钮
					if(!option.auto){
						$html.find('.h-uploadbtn').css('display','inline-block');
					}
					uploadManager._getFileList().append($html);

					//判断是否显示已上传文件大小
					if(option.showUploadedSize){
						var num = '<span class="h-progressnum"><span class="uploadedsize">0KB</span>/<span class="totalsize">${fileSize}</span></span>'.replace(/\${fileSize}/g,F.formatFileSize(file.size));
						$html.find('.h-uploadify-progress').after(num);
					}
					
					//判断是否显示上传百分比	
					if(option.showUploadedPercent){
						var percentText = '<span class="h_up_percent">0%</span>';
						$html.find('.h-uploadify-progress').after(percentText);
					}
					...
					if(option.auto){
						...
						//如果配置非自动上传，绑定上传事件
					 	$html.find('.h-uploadbtn').on('click',function(){
					 		if(!$(this).hasClass('.h-disabledbtn')){
					 			$(this).addClass('.h-disabledbtn');
					 			uploadManager._uploadFile(file);
					 		}
				 		});
					}
					...
					//为删除文件按钮绑定删除文件事件
			 		$html.find('.h-delfilebtn').on('click',function(){
			 			if(!$(this).hasClass('.h-disabledbtn')){
					 		$(this).addClass('.h-disabledbtn');
				...
				_getFiles : function(e){
			  		...
			  		var fileCount = _this.find('.h-uploadify-queue .h-uploadify-queue-item').length;//队列中已经有的文件个数
		  			for(var i=0,len=files.length;i<len;i++){
				...
				_regulateView : function(file){
					...
					thisfile.find('.h-uploadify-progress-bar').css('width','100%');
					option.showUploadedSize && thisfile.find('.uploadedsize').text(thisfile.find('.totalsize').text());
					option.showUploadedPercent && thisfile.find('.h_up_percent').text('100%');	
				},
				onProgress : function(file, loaded, total) {
					var eleProgress = _this.find('#fileupload_'+instanceNumber+'_'+file.index+' .h-uploadify-progress');
					var percent = (loaded / total * 100).toFixed(2) +'%';
					if(option.showUploadedSize){
						eleProgress.nextAll('.h-progressnum .uploadedsize').text(F.formatFileSize(loaded));
						eleProgress.nextAll('.h-progressnum .totalsize').text(F.formatFileSize(total));
					}
					if(option.showUploadedPercent){
						eleProgress.nextAll('.h_up_percent').text(percent);	
					}
					eleProgress.children('.h-uploadify-progress-bar').css('width',percent);
			  	},
