		...
		validateField: function( event, data ) {
			...
			if ( data.notComponent ) {
				...
				if ( "radio" === type || "checkbox" === type ) {
					...
					try {
						val = el[type]("getValue");// default way to getValue method
					} catch (e){
			...
			if ( errorResults.length > 0 ) {
				...
				if ( !data.notComponent ) {
					if( errorMode == 0 ){
						...
						component.append("<span class='icon icon-info coral-errorIcon' style='right:"+paddingRight+"' data-errors = '"+errorResults.toString()+"' ></span>")											
...
//			},
		...
		showTooltip: function (el, validTypeOptions){
			var validTypeOptions = $.extend(true,{},$.validate.validTypeOptions,validTypeOptions),
			    showTooltip = validTypeOptions.maxlength.showTooltip;			
			...
			$( el ).tooltip("option","content","还可以输入"+maxlength+"个字符");
			el.keyup(function(){
				var val = $( el ).val(),
	    ...
	    **/
		...
		_onFieldOptionChange: function( $field, e, ui ) {
			...
			if ( ui.key == "required" ) {
	        	...
	        	$field[type]("component").removeClass( "coral-validate-error" );
	        	$field[type]("component").removeAttr( "data-errors" ); 
	        	$( ".coral-errorIcon", $field[type]("component") ).remove();
		        // clear error states
	        	$field.prop( "isError", false );
			}
		},		
