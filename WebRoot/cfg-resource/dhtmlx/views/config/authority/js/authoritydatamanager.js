...
function initDataSelfDefineMenuTree(objId, objType) {
    var treeJson;
    var SELF_DEFINE_MENU_TREE_URL = contextPath
    ...
    if ("1" == objType) {
        ...
        SELF_DEFINE_MENU_TREE_URL += "&systemId=" + currentSystemId;
        treeJson = {
                id : 0,
                item : [{
                    id : currentSystemId,
                    text : currentSystemName,
                    im0 : "safe_close.gif",
                    im1 : "safe_open.gif",
                    im2 : "safe_close.gif",
                    item : []
    ...
    if ("1" == objType) {
        ...
        dataSelfDefineMenuTree.refreshItem(currentSystemId);
    }
    dataSelfDefineMenuTree.attachEvent("onClick", function(nId) {
        ...
        if (bindingType == "1") {
            var url = COMMON_AUTHORITY_TREE_URL + "!getDataConfigGridData.json?P_objectId="
...
function treeAuthority(objectId, objectType, menuId, componentVersionId) {
    var treeId = loadJson(COMMON_AUTHORITY_TREE_URL + "!getTreeRootId.json?P_componentVersionId=" + componentVersionId);
    if (isNotEmpty(treeId)) {
        var treeNameJson = loadJson(TREE_DEFINE_URL + "!show.json?id=" + treeId);
