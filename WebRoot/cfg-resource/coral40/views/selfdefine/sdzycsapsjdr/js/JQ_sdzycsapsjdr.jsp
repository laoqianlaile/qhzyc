...
window[CFG_actionName(subffix)] = function (ui) {
	...
	return jQuery.contextPath + "/sdzycsapsjdr";
};
...
function _override_form (ui) {
	...
	ui._init = function (){
		//初始化图片删除区域
		hUI= ui;
		var jywjDiv = ui.getItemBorderJQ("scwj");
		jywjDiv.empty();
		jywjDiv.html(initWJdiv());

		var sctpDiv = ui.getItemBorderJQ("yptp");
		sctpDiv.empty();
		sctpDiv.html(initTPdiv());
		initToolbar();
		...
		if(!isEmpty(formData.YPTP))
			$.ns("ns"+subffix+"").uploadImage($.contextPath+"/spzstpfj/"+formData.YPTP);
	}
	...
	ui.addOutputValue("setCombogridValue_ycmc", function (o){
		...
		ui.clickSave = function( op ){
			...
			var dfile = document.forms[jqForm.attr('id')]["excelUpload"].files[0];
			if ( undefined != dfile && 'undefined' != dfile)
				postData.append("uploadFile",dfile);

//			var dimage = document.forms[jqForm.attr('id')]["imageUpload"].files[0];
//			if ( undefined != dimage && 'undefined' != dimage)
//				postData.append("imageUpload",dimage);
			$.ajax({
				type : "POST",
				url : url,
				data : postData,
				...
				success : function(entity) {
					if (false === entity.success || !("ID" in entity)) {
						if (isEmpty(entity.message)) CFG_message("读取Excel失败！", "warning");
...
function _override_grid (ui) {
	...
	//	return setting;
	//};
	ui.addOutputValue("setSapjsdr_value",function ( res ) {
		var id = ui.getSelectedRowId();
		return {status: true};
	});
...
function _override_bsearch (ui) {
	...
	//};
	ui._init = function(){
		//ui.getItemJQ("FZR").combogrid({width:30});
		var button = document.getElementById("PCH_button");
...
jQuery.extend(jQuery.ns("ns" + subffix), {
	...
	viewExcel : function (fileInput) {
		var file = fileInput.files;
		var f = file[file.length-1];
		var fileSuffix = f.name.substring(f.name.lastIndexOf('.')+1).toLowerCase();

		//图片不能超过20M，20971520Bytes
		if(!(f.size<20971520 && (fileSuffix=='xls'||fileSuffix=='xlsx'))){
			CFG_message("文件必须小于20M，且格式必须为.xls或.xlsx！", "error");
...
//            div.style.marginTop = "10px";
	...
	deleteImage : function (src) {
		...
		$.ajax({
			type: "POST",
			url: $.contextPath + '/sdzycsapsjdr!deleteTp.json',
			data: {'id': hUI.options.dataId},
			dataType: "json",
			success: function (data) {
				var view = $("#view${idSuffix}");
				hUI.setFormData({"YPTP":""});
				view.empty();
				$("#imageUpload${idSuffix}").val("");
	...
	function  initWJdiv() {
		var html = "<div style=\"margin-bottom: 50px;margin-left:55px\">"+
				...
				"<label class=\"app-input-label\" style=\"width:20%;\">SAP数据导入：</label>"+
				"<div id=\"file${idSuffix}\">"+
				"<input class=\"inputfile\" type=\"file\" style=\"width:300px;\" id=\"excelUpload${idSuffix}\" multiple=\"multiple\" lable=\"预览\"	onChange=\"jQuery.ns('ns" + subffix+"').viewExcel(this)\" name=\"uploadFile\" />"+
				"</div>"+
				"</div>"+
				"</div>"+
				"</div>";
	...
	function initTPdiv(){
		var html = "<div style=\"margin-bottom: 50px;margin-left:55px\">"+
				"<div class=\"fillwidth colspan2 clearfix\">"+
				"<div class=\"app-inputdiv6\">"+
				"<label class=\"app-input-label\" style=\"width:20%;\">饮片图片：</label>"+
				"<div id=\"file1${idSuffix}\">"+
