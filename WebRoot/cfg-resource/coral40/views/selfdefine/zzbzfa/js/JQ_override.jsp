...
<script type="text/javascript">
    ...
    (function (subffix) {
        ...
        /**
         * 刷新子列表标记：由于组件库列表没有列表渲染完成的事件，
         * 所以在子列表onComplele事件中去加载主列表的数据加载事件，
         * 并把相关的条件传回给子列表后，重新刷新子列表（渲染），
         * 不能用重新加载数据（在子列表onComplete事件没执行完时，组件库列表无法重新加载数据，因此，用暴力重新渲染）
         * 注意：从列表的setting.datatype是根据这个标记来判断
         *
         *
         * 还有一个方法：如果你要试试 你明天到公司找我
         */
...
//                var detailGrid = ui.getDetailGrid();
                    ...
                    var zpfsIndex = $.inArray("栽培方式", setting.colNames);
                    setting.colModel[zpfsIndex].formatter = 'combobox';
                    setting.colModel[zpfsIndex].revertCode = true;//存编码
                    ...
                    //用量
                    var ylIndex = $.inArray("用量", setting.colNames);
                    setting.colModel[ylIndex].formatter = 'text';
                    setting.colModel[ylIndex].formatoptions = {
                    };
                    //农事作业项名称
...
//        //获取表单配置信息
//        var formConfigInfo = ui.assembleData.parentConfigInfo.parentConfigInfo.parentConfigInfo.topChildConfigInfo;
//        //主表数据id
//        var pid = formConfigInfo.dataId;
//		if(id == $.config.preButtonId + "save"){
//			//调用主表表单保存方法
//			$("#"  + formConfigInfo.cformDivId).cform("clickSave");
//            //更新主表配置及pid
//            formConfigInfo = ui.assembleData.parentConfigInfo.parentConfigInfo.parentConfigInfo.topChildConfigInfo;
//            pid = formConfigInfo.dataId;
//		}
                    if (id == $.config.preButtonId + "add") {
                        ...
                        $grid.grid("addRowData", rowId, {
                            ID: rowId,
                            PID: _faid,
                            NSZYXBH: currentMaxNo + 1,
                            IS_START: "0",
                            IS_START1:"否",
                            CFJGSJ:"0",
                            CFCS:1,
                    ...
                    $.confirm("当前记录将从列表移除，确定吗？", function (sure) {
                        if (sure) {
                            ...
                            if (id.substr(0, 5) == "temp_") {
                                ...
                                var delStatus = $.loadJson($.contextPath + '/zzbzfa!deleteBzfa.json?dataId=' + id);
                                if (delStatus == "SUCCESS") _this.uiGrid.grid("delRowData", id);
                                else CFG_message("删除失败", "error");
                            }
                            for (var i = detailGridRowData.length - 1; i >= 0; i--) {
                                if (nsxbh == detailGridRowData[i].NSZYXBH) {
                                    if (detailGridRowData[i].ID.substr(0, 5) == "temp_") {
                                        ...
                                        var delStatus = $.loadJson($.contextPath + '/zzbzfa!deleteBzfaTrp.json?dataId=' + detailGridRowData[i].ID);
                ...
                ui.beforeInitGrid = function (setting) {
                    ...
                    var trpxx = $.loadJson($.contextPath + '/zzbzfa!getTrpxx.json?lx=bzfa');
                    var trpTymArray = [];
                    for (var i in trpxx) {
                        trpTymArray.push({'text': trpxx[i].ZZZMMC, 'value': trpxx[i].ZZZMBH});
                    }
                    //投入品通用名
...
//                                    ui.uiGrid.grid("setCell", e.data.rowId, "YT", trpxx[i].YT);
                    ...
                    var zyxbhArray = [];
                    if(_faid != false && _faid != ""){
                        zyxbhArray = $.loadJson($.contextPath + "/zzfalb!getZyxbh.json?id=" + _faid + "&tablename=T_ZZ_BZFA");
                    }
                    ...
                    $.confirm("当前记录将从列表移除，确定吗？", function (sure) {
                        if (sure) {
                            if (id.substr(0, 5) == "temp_") {
                                ...
                                var delStatus = $.loadJson($.contextPath + '/zzbzfa!deleteBzfaTrp.json?dataId=' + id);
                                if (delStatus == "SUCCESS") _this.uiGrid.grid("delRowData", id);
                                else CFG_message("删除失败", "error");
                            }
                        }
