    ...
    <div class="fill">
        ...
        <div class="toolbarsnav clearfix">
            <ces:toolbar id="toolbarId2${idSuffix}" onClick="$.ns('namespaceId${idSuffix}').toolbarClick"
                         data="['->',{'label': '添加农事项', 'id':'addTrply', 'disabled': 'false','type': 'button'},'','','','','','','','']">
            </ces:toolbar>
        </div>
        <ces:grid id="pGridId${idSuffix}"
            ...
            <ces:gridCols>
                ...
                <ces:gridCol name="KCSL"  width="120" formatter="text" >库存重量</ces:gridCol>

                <%--<ces:gridCol name="GYSBH"  width="120" formatter="text" editable="true" formatoptions="readonly:false" >供应商编号</ces:gridCol>--%>
                <ces:gridCol name="CKSL"  width="120" formatter="text" editable="true" formatoptions="required: true" >出库数量</ces:gridCol>
    ...
    $(function() {
        var configInfo = CFG_initConfigInfo({
            ...
            'bodyOnLoad' : function(configInfo) {
                ...
                $("#pGridId${idSuffix}").grid("setColProp", "GYSMC",{"formatoptions":{"textField":"name","valueField":"value",
...
//                        alert('进入onchange');
                ...
                $("#pGridId${idSuffix}").grid("setColProp", "RKLSH",{"formatoptions":{"textField":"RKLSH","valueField":"RKLSH",
//                    onChange:function(ui, data) {
...
//            var scpch=getgridData.data[len].SCPCH;
                        var scpch=11;
                        var gysData=$.loadJson($.contextPath + "/zztrplylb!getGysData.json?trpbh="+trpbh+"&scpch="+scpch);
                        var c = $("#pGridId${idSuffix}").grid("getCellComponent", $griddata.data[len].ID, "GYSMC");
                        c.combobox("option",
                                {
                                    "data": gysData,
                                    "textField":"GYSMC",
                        ...
                        c.combobox("setValue", gysData[0].GYSBH);
                    ...
                    onClick: function(e, ui) {
                        if ("save" === ui.id) {//执行保存操作
                            ...
                            $.each(sel, function(i){
                                ...
                                zbgrid.grid('addRowData',lensd+i,rd);

                                var gysData=$.loadJson($.contextPath + "/zztrplylb!getGysData.json?trpbh="+rd.TRPBH+"&scpch="+"");
                                $("#pGridId${idSuffix}").grid("getCellComponent", lensd+i, "GYSMC").combobox({
                                    textField:"GYSMC",
                                    valueField:"GYSBH",
                                });
                                $("#pGridId${idSuffix}").grid("getCellComponent", lensd+i, "GYSMC").combobox("reload", gysData);
                                $("#pGridId${idSuffix}").grid("getCellComponent", lensd+i, "GYSMC").combobox("setValue", gysData[0].GYSBH);

                                var gysbh=gysData[0].GYSBH;
                                if(gysbh==''){return;}
                                var pchData=$.loadJson($.contextPath + "/zztrplylb!getScpchData.json?trpbh="+rd.TRPBH+"&gysbh="+gysbh);

                                $("#pGridId${idSuffix}").grid("getCellComponent", lensd+i, "RKLSH").combobox({
                                    textField:"RKLSH",
                                    valueField:"RKLSHBH",
                                });
                                $("#pGridId${idSuffix}").grid("getCellComponent", lensd+i, "RKLSH").combobox("reload", pchData);
                                $("#pGridId${idSuffix}").grid("getCellComponent", lensd+i, "RKLSH").combobox("setValue", pchData[0].RKLSHBH);
                                var ckslData=$.loadJson($.contextPath + "/zztrplylb!getCkslData.json?rklsh="+pchData[0].RKLSH);
                                $("#pGridId${idSuffix}").grid("getCellComponent", lensd+i, "KCSL").textbox('setValue',ckslData.KCSL);
