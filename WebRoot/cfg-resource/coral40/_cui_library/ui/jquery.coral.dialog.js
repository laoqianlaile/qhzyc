...
$.component( "coral.dialog", {
	...
	_create: function() {
		...
		this.element.addClass( "ctrl-init ctrl-init-dialog" );
		...
		switch ( this.options.wtype ) {
			...
			case "dialog":
				this._createTitlebar();
				this._createLoading(); // 20141230 - haibo lee
				this._createButtonPanel();
		}
	...
	close: function( event ) {
		...
		this._hide( this.uiDialog, this.options.hide, function() {
			...
			if ( that.opener.length && that.opener[0].tagName.toLowerCase() != "object" && !that.opener.filter(":focusable").focus().length ) {
	...
	open: function() {
		...
		if ( canRequest ) {
			//that.showLoading(); // loading begin
		...
		if ( !canRequest ) {
			$.coral.refreshAllComponent(this.element);//added 
		}
		...
		this._on( this.uiDialog, {
			keydown: function( event ) {
				if ( this.options.closeOnEscape && !event.isDefaultPrevented() && event.keyCode &&
						event.keyCode === $.coral.keyCode.ESCAPE ) {
					event.preventDefault();
					this.close( event );
					return;
				}
	...
	_createLoading: function () {
		...
		this.uiDialogLoading = $("<div class='coral-dialog-loading'>" + opts.loadtext + "</div>").appendTo( this.uiDialog );
	},
	/**
	 * added @haibo lee
	 *  显示loading dom
	...
	_createTitlebar: function() {
		...
		this._on( this.uiDialogTitlebar, {
			mousedown: function( event ) {
				if ( !$( event.target ).closest(".coral-dialog-titlebar-close") 
						&& !$( event.target ).closest(".coral-dialog-titlebar-maximum")) {
					this.uiDialog.focus();
				}
			}
		});
		...
		if (this.options.maximizable) {
			...
			this.uiDialogTitlebarMaximum = $( "<button type='button'></button>" )
				.button({
					...
					icons: {
						primary: "icon-expand3"
					},
					text: false
				})
				.addClass("coral-dialog-titlebar-maximum")
		...
		if (this.options.closable) {
			this.uiDialogTitlebarClose = $( "<button type='button'></button>" )
				...
				.addClass("coral-dialog-titlebar-close")
				.appendTo( this.uiDialogTitlebar );
	...
	maximize: function(isInit){
		...
		$(that.element).dialog("option", "width", "100%");
		$(that.element).dialog("option", "height", "100%");
		that.uiDialog.addClass("coral-dialog-maximum");
		that.uiDialogTitlebarMaximum.find(".icon-expand3").removeClass("icon-expand3").addClass("icon-contract3");
	},
	...
	restore: function(){
		...
		that.uiDialogTitlebarMaximum.find(".icon-contract3").removeClass("icon-contract3").addClass("icon-expand3");
	},	
	...
	_makeDraggable: function() {
		...
		this.uiDialog.draggable({
			cancel: ".coral-dialog-content, .coral-dialog-titlebar-close",
