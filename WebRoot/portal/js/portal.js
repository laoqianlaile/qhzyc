...
document.write('<link href="'+basePath+'/portal/css/portal.css" rel="stylesheet" type="text/css"/>');
...
(function($){
	...
	function setSize(target){
		...
		if (!isNaN(opts.width)){t._outerWidth(opts.width);} else {t.width('auto');}
		if (!isNaN(opts.height)){t._outerHeight(opts.height);} else {t.height('auto');}
		
		// var hasScroll = t.find('>table').outerHeight() > t.height();// modify by chanlong on 2014-12-04 11:12
		var width = t.width();
		...
		for(var i=0; i<columnWidths.length; i++){
			...
			var h = p.find('div.portal-p').length;
			// modify by chanlong on 2014-12-04 11:12
			p.find('div.portal-p').panel('resize', {width:p.width(),height:t.height()/h-opts.fixheight});
		}
