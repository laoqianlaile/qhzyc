...
<div id="maxDiv${idSuffix}" class="fill">
  ...
  <div class='homeSpan' style="margin-top: -23px;"><div><div style='margin-left:20px;width: 150px;' id="nva${idSuffix}"> - 订单管理 - 新增 </div></div></div>
  <div id="ddFormDiv${idSuffix}" style="margin-left: 20px;overflow: auto;height: 325px; ">
    <form id="ddForm${idSuffix}" enctype="multipart/form-data" method="post" class="coralui-form">
      <div class="fillwidth colspan3 clearfix">
        ...
        <div class="app-inputdiv4">
          ...
          <input id="SHSJ${idSuffix}" name="SHSJ">
        </div>
    ...
    <div id="shdd${idSuffix}">
      ...
      <ces:grid id="shddGrid${idSuffix}" height="auto" rownumbers="true" fitStyle="fill" datatype="local">
        <ces:gridCols>
          ...
          <ces:gridCol name="PL" formatter="combobox" formatoptions="required: true"
                       width="80">品类</ces:gridCol>
          <ces:gridCol name="PZ" formatter="combobox" formatoptions="required: true,'textField':'name','valueField':'value'"
                       width="80">品种</ces:gridCol>
          <ces:gridCol name="ZL" formatter="text" formatoptions="required: true"
    ...
    <div id="cpdd${idSuffix}">
      ...
      <ces:grid id="cpddGrid${idSuffix}" fitStyle="fill"  height="auto" rownumbers="true" datatype="local" >
        <ces:gridCols>
          ...
          <ces:gridCol name="CPMC" width="100" formatter="combobox" formatoptions="required: true" sortable="true" >产品名称</ces:gridCol>
          <ces:gridCol name="BZXS" width="80" formatter="combobox" formatoptions="required: true,readonly:true">包装形式</ces:gridCol>
          <ces:gridCol name="CPGG" width="80" >产品规格</ces:gridCol>
          <ces:gridCol name="CPSL" width="80" formatter="text" >产品数量</ces:gridCol>
          <ces:gridCol name="CZ" formatter="toolbar"
...
<script>
  ...
  $.extend($.ns("namespaceId${idSuffix}"), {
    khClick:function(){
      ...
      <%--$('#MDMC${idSuffix}').combogrid("option","disabled",false);--%>
      <%--var mdData = $.loadJson($.contextPath + "/tcszzmdxx!searchGridData.json?khbh=" + $('#KHBH${idSuffix}').combogrid('getValue'));--%>
      <%--$('#MDMC${idSuffix}').combogrid("reload",mdData.data );--%>
      <%--setKhxx(data.value);--%>
    },
    setComboGridValue_khxx : function ( o ){
      ...
      $('#MDMC${idSuffix}').combogrid("reload",mdData.data );
      $('#MDLXR${idSuffix}').textbox('setValue','');
      $('#MDLXDH${idSuffix}').textbox('setValue','');
      $('#MDDZ${idSuffix}').textbox('setValue','');
      //根据ID加载默认带
      setKhxx(obj.KHBH);
    },
    mdClick : function(){
  ...
  $(function () {
    var configInfo = CFG_initConfigInfo({
      ...
      'bodyOnLoad': function (configInfo) {
        ...
        //初始化包装形式
        $cpddgrid.grid("setColProp", "BZXS",{"formatoptions":{"data":bzxsData,"textField":"text","valueField":"value",required:true,readonly:true}});
        $shddgrid.grid("setColProp", "PL",{"formatoptions":{"data":plData,"textField":"name","valueField":"value",onChange:function(ui, data) {
...
//        }}});
        ...
        $cpddgrid.grid("setColProp", "CPMC",{"formatoptions":{"data":cpDta,"textField":"CPMC","valueField":"CPBH",onChange:function(ui, data) {
          //根据产品编号获得对应产品信息
        ...
        if(isEmpty(inputParamId)){//新增时默认初始化
          ...
          $shddgrid.grid("option","onComplete",function( e, ui){
            ...
            for(var i = 0 ; i < ids.length ; i++){
              ...
              $pzCombobox.combobox("reload", pzData);
              if(driddata.data[i] == undefined) return;
