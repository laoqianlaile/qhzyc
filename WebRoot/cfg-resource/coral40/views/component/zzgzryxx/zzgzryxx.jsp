...
<div id="max${idSuffix}" class="fill">
    <div id="ft${idSuffix}" class="toolbarsnav clearfix top0">
        ...
        <div class='homeSpan' style="margin-top: -23px;"><div><div style='margin-left:20px;width: 150px;' id="nva${idSuffix}"> -生产人员信息-新增 </div></div></div>
    </div>
    <form id="enterForm${idSuffix}" action="${basePath}"
        ...
        <div class="fillwidth colspan3 clearfix">
                <label class="app-input-label" style="width:20%;display: none">工作人员编号:</label>
                <input id="gzrybh${idSuffix}" data-options="readonly:true" name="gzrybh" type="hidden" class="coralui-textbox"/>
        ...
        <div class="fillwidth colspan3 clearfix">
            <div class="app-inputdiv4">
                ...
                <input id="xb${idSuffix}" name="xb"  class="coralui-radiolist" data-options="value:'1',column:2,data:[{value:'1',text:'男'},{value:'2',text:'女'}]">
            </div>
            <div class="app-inputdiv4">
...
<script type="text/javascript">
    ...
    $("#enterForm${idSuffix}").submit(
            function () {
                ...
                dwlx = dqxtlxJsonData.code.toUpperCase();
                var formdata = new FormData(this);
                $.ajax({
                    type: 'POST',
                    url: "${basePath}zzgzryda!saveGzryxx.json?dwlx="+dwlx,
                    data: formdata,
                    /**
...
//                var gw_data = $.loadJson($.contextPath + "/zzgzryda!getGwBySjzd.json");
                var gw_data = $.loadJson($.contextPath + "/zzgzryda!getGwByDwlx.json?dwlx="+dwlx);
                $("#gw${idSuffix}").combobox("reload", gw_data);
                var xb_data = $.loadJson($.contextPath + "/zzgzryda!getXbBySjzd.json");
                //$("#xb${idSuffix}").radiolist("setValue", xb_data);
                ...
                if(rowDataId !== "" && rowDataId !== null){
                    ...
                    $("#xb${idSuffix}").radiolist("setValue",
        ...
        $("#xm${idSuffix}").blur(function(event, ui){
                var  xm=$("#xm${idSuffix}").val();
                  var  xm=$("#xm${idSuffix}").val();
                var gzryxm =$.loadJson($.contextPath + "/zzgzryda!checkGZRYMC.json?gzryxm="+xm+"&dwlx="+dwlx);
            if(gzryxm.XM > 0){
                CFG_message("该人员已经存在，请重新输入", "error");
                $("#xm${idSuffix}").textbox("setValue","");
                return false;
            }
        });
