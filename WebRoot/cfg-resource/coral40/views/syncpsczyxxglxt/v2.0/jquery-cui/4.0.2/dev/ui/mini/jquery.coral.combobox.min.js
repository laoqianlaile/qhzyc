...
if(!this.currentValues||(!this.currentValues[0]&&this.currentValues.length===1)){return d}for(;h<this.currentValues.length;h++){var k=this.currentValues[h],f=0,l=c.valueField,e=c.textField,m=null;if("value-text"===c.postMode){k=k.split(c.valueTextSeparator)[0];d.push(k)}if("value"===c.postMode){d.push(k)}if("text"===c.postMode){for(;g&&f<g.length;f++){m=g[f];if(m[e]==k){d.push(m[l]);break}}}}return d},_getCurrentValues:function(){var f=this.getData(),e=this.options,d=[],c=0;if(!this.currentValues||(!this.currentValues[0]&&this.currentValues.length===1)){return d}return this.currentValues},getData:function(){return this.data||[]},setValues:function(c,q,h){var p=this.options,B=this.getData(),m=this.panel(),u=h,w=0,v=0,A=this.getValues()||[],l=[],n=[],x=null,t=null,o=null,r=0,s=0;if(typeof(u)=="object"){h=u.remainText;q=u.triggerOnChange}this.currentValues=c;if(!this.dataLoaded){return}m.find(".coral-combobox-item-selected").removeClass("coral-combobox-item-selected");for(w=0;w<c.length;w++){t=c[w];o=t;var g=this.getRowIndex(t);if(g>-1){o=B[g][p.textField];x=B[g][p.valueField];var f=this._getItemByIndex(g).addClass("coral-combobox-item-selected");if(p.emptyText&&""==t){this.uiCombo.textbox.attr("placeholder",p.emptyText);this._showPlaceholder(p.emptyText);o=""}else{this._hidePlaceholder()}n.push(o);l.push(t)}else{if(g<0&&!p.forceSelection){n.push(o);l.push(t)}}}if(!h){this._setText(n.join(p.separator))}var k=this._getMinus(l,c);if((k.length&&l.length)||!l.length){n=n.concat(k);if(!h){this._setText(n.join(p.separator))}l=c}if(p.width=="item"){var d=a("<div style = 'visibility:hidden;'><span>"+this.getText()+"</span></div>").appendTo("body"),e=this.component().find(".coral-textbox-default");var z=parseInt(e.css("padding-left"))+parseInt(e.css("padding-right")),y=this.uiArrow().outerWidth()+2*parseInt(this.uiArrow().css("right")),s=d.find("span").outerWidth()+z+y;this.resize(s);d.remove();p.width="item"}this._super(l,q,h)},_getMinus:function(d,c){var e=[];a.each(c,function(f,g){if(a.inArray(g,d)==-1){e.push(g)
