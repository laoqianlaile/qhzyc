		...
		for (var i = 0; i < colModel.length; i++) {
			...
			if ("TPBCMC" == obj.name) {
				setting.colModel[i].formatter = function (value, options, rowObj) {
					var dz = rowObj["TPBCMC"];
					var src = $.contextPath + '/spzstpfj/' + dz;
					return "<img src='" + src+"' width='200'/>";
				}
			}
		}
		setting.picTemplate = function (rData, ni, multi) {
			var rowData = rData.rowData;
			var url = $.contextPath + '/spzstpfj/' + rowData["TPBCMC"];
			var itemJson = {}, itemData = [],
					ccols = ui.gcfg.cardColumns,
					i     = 0;
			...
			if (ccols && ccols.length > 0) {
				...
				itemData.push("<div>" + rData[ui.gcfg.columns[0]] + "</div>");
			}

			itemJson["coreData"]    = "<img src=\"" + url + "\">";
			itemJson["itemData"]    = rowData["SCTPMC"];
			return itemJson;
		};
		return setting;
	}
