package com.ces.component.sdzyccjgylrkxx.action;

import com.ces.component.sdzyccjgylrkxx.dao.SdzyccjgylrkxxDao;
import com.ces.component.sdzyccjgylrkxx.service.SdzyccjgylrkxxService;
...
@Results({@Result(name = "downLoad", type = "stream", params = {
        ...
        "bufferSize","4096" })})
public class SdzyccjgylrkxxController extends TraceShowModuleDefineServiceDaoController<StringIDEntity, SdzyccjgylrkxxService, SdzyccjgylrkxxDao> {
     ...
     */
    public void searchQypchGridData(){setReturnData(getService().getYlllpchGridData());}
     ...
     * 获取企业名称
     */
    public void searchQybm(){
        String qybm = getParameter("qybm");
        String dwlx = getParameter("dwlx");
        setReturnData(getService().searchDataByQybm(qybm,dwlx));
    }
    /**
     * 查询输出值是否大于粗加工的入库重量
     */
...
//                book = new XSSFWorkbook(new FileInputStream(realpath + "/" + fileNameWithStamp));
        ...
        for (int i = sheet.getRow(0).getFirstCellNum() ; i < sheet.getRow(0).getLastCellNum(); i++) {
            ...
            for (int j = sheet.getFirstRowNum() + 1; j <= sheet.getLastRowNum(); j++) {
                ...
                if (dataList.size() < j) {
                    ...
                    dataList.get(j - 1).put("YLMC","");
                    dataList.get(j - 1).put("YLCD","");
                    dataList.get(j - 1).put("GYS","");
                    dataList.get(j - 1).put("RKZL","");
                    dataList.get(j - 1).put("RKSJ","");
                    dataList.get(j - 1).put("RKDJFZR","");
    ...
    public Object destroy() throws FatalException {
        try {
            String ids      = getId();
            String sqlc="select t.pch,t.id from  t_sdzyc_cjg_ylrkxx t where id IN ('" + ids.replace(",", "','") + "')";
