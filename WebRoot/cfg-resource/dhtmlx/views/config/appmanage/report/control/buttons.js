...
function onCbClick(szCommand, fState)
{
	...
	switch(szCommand.toUpperCase())
	{
		...
		case "CMDFILEOPEN"://打开远程文件
			...
			CellUtil.reset();
			break;
		case "CMDFILEWEBOPEN"://打开远程文件
			mnuFileWebOpen_click();
			CellUtil.reset();
			break;
		case "CMDFILESAVE"://保存文档
			CellUtil.save();//mnuFileSave_click();
			break;
		case "CMDFILEPRINT"://打印文档
			CellWeb.PrintSheet( false, CellWeb.GetCurSheet() );
			break;
		case "CMDFILEPAGESETUP"://打印设置
