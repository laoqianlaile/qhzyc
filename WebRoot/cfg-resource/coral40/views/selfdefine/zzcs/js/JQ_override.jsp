...
//                var detailGrid = ui.getDetailGrid();
                    ...
                    //采收方式
                    var csfsIndex = $.inArray("采收方式",setting.colNames);
                    var csfs = $.loadJson($.contextPath + "/trace!getDataDictionary.json?lxbm=ZZSCCSFS");
                    setting.colModel[csfsIndex].formatter = 'combobox';
                    setting.colModel[csfsIndex].revertCode = true;//存编码
                    setting.colModel[csfsIndex].formatoptions = {
                        'emptyText': '请选择',
                        'required': true,
                        'data' : csfs,
                        'readonly' : isReadonly
                    ...
                    $.confirm("当前记录将从列表移除，确定吗？", function (sure) {
                        if (sure) {
                            ...
                            if (id.substr(0, 5) == "temp_") {
                                ...
                                var delStatus = $.loadJson($.contextPath + '/zzcs!deleteCs.json?dataId=' + id);
                            ...
                            for (var i in detailGridRowData) {
                                if (nsxbh == detailGridRowData[i].NSZYXBH) {
                                    if (detailGridRowData[i].ID.substr(0,5) == "temp_") {
                                        ...
                                        var delStatus = $.loadJson($.contextPath + '/zzcs!deleteCsTrp.json?dataId=' + detailGridRowData[i].ID);
                ...
                ui.beforeInitGrid = function (setting) {
                    ...
                    var zyxbhArray = [];
                    if(_faid != false && _faid != ""){
                        zyxbhArray = $.loadJson($.contextPath + "/zzfalb!getZyxbh.json?id=" + _faid + "&tablename=T_ZZ_CS");
                    }
                    ...
                    $.confirm("当前记录将从列表移除，确定吗？", function(sure) {
                        if (sure) {
                            if(id.substr(0,5)=="temp_") {
                                ...
                                var delStatus = $.loadJson($.contextPath + '/zzcs!deleteCsTrp.json?dataId='+id);
                                if(delStatus == "SUCCESS") _this.uiGrid.grid("delRowData", id);
                                else CFG_message("删除失败", "error");
                            }
                        }
