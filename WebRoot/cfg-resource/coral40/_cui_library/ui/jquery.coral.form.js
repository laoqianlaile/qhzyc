...
( function ( $, undefined ) {
	...
	$.component( "coral.form", {
		...
		_create: function () {
			var that = this;
			//创建组件外围元素
			this.uiForm = $( "<div class=\"coral-form\"></div>" );
			this._initElement();
			this._bindEvents();
		...
		_initElement: function () {
			...
			//添加组件外围元素
			this.uiBorder =  $( "<div class=\"coral-form-border\"></div>" );		
			this.uiForm.addClass( options.cls );
			this.uiForm.append( this.uiBorder );
			this.uiForm.insertAfter( this.element );		
		 ...
		 * 取得表单的数据
		 * */
		getData: function ( isSerialize ) {
			return this.formData( isSerialize );
		},
		...
		_getForm: function() {
			...
			$( arr ).each( function(index, item) {
				if ( obj[item.name] ) {
					obj[item.name] = obj[item.name] + that.options.separator + item.value;
				} else {
		...
		_loadCoralData: function ( key, value ) {
			...
			for ( var i in coralData ) {
				...
				switch ( item.coralType ) {
					case "radio": 
						if ( key == item.name ) {
							isFind =  true;				
							if ( item.el.radio("option", "value") == value && value != null ) {
								item.el.radio( "check" );
							} else {
								item.el.radio( "uncheck" );
							}		
						}
						break;
					case "radiolist": 
						if ( key == item.name ) {
							isFind =  true;	
							if (value == null ) value = "";	
							item.el.radiolist( "setValue", value );
						}
						break;
					case "checkbox":
						if ( key == item.name ) {
							...
							for ( var a in value ) {
								if ( item.el.checkbox("option", "value") == value[a] ) {
									item.el.checkbox( "check" );
									break;
								} else {
									item.el.checkbox( "uncheck" );
								}
							}											
						}
						break;
					case "checkboxlist":
						if ( key == item.name ) {
							...
							if ( key == item.name ) {
								if ( value == null ) value = "";
								item.el.checkboxlist( "setValue", value );
							} 
						}
						break;
					case "combobox": 
						if ( key == item.name ) {
							...
							if ( value != null ) {
								item.el.combobox( "setValues", value );
							}						
						}
						break;
					case "combogrid": 
						if ( key == item.name ) {
							...
							if ( value != null ) {
								item.el.combogrid( "setValues", value );
							}						
						}
						break;
					case "combotree":
						if ( key == item.name ) {
							...
							if ( value != null ) {
								item.el.combotree( "setComboValues", value );
							}						
						}
						break;
					case "datepicker": 
						if ( key == item.name ) {
							isFind =  true;	
							if ( value != null ) {
								item.el.datepicker( "option", "value", value );
							}
						}
						break;
					case "autocomplete": 
						if ( key == item.name ) {
							isFind =  true;	
							if ( value != null ) {
								item.el.autocomplete( "option", "value", value );
							}
						}
						break;
					case "autocompletetree": 
						if ( key == item.name ) {
							isFind =  true;	
							if ( value != null ) {
								item.el.autocompletetree( "option", "value", value );
							}
						}
						break;
					default:
						if ( key == item.name ) {
							isFind =  true;	
							if ( value != null ) {
								item.el[item.coralType]( "option", "value", value );
							}
						}
						break;
				}
		...
		reset: function () {
			var that = this,
				originalData = $.data( this.element, "originalData" );
			
			that._loadData( originalData );
			$.data( this.element, "modifiedData", {} );
		},
		...
		setIsLabel: function(isSet){
			var fields = $.coral.findComponent( ".ctrl-form-element", this.element ),
				i = 0, 
				l = fields.length;
			for (i; i < l; i++) {
		        var c = fields[i];
		...
		refresh: function(opts){	
			...
			parent = this.uiForm.parent();
			if ( heightStyle === "fill" ) {
				maxHeight = parent.height();
				this.uiForm.siblings( ":visible" ).each(function() {
					var elem = $( this ),
