...
window[CFG_actionName(subffix)] = function (ui) {
	...
	return jQuery.contextPath + "/sdzyccscggl";
};
...
function _override_form (ui) {
	ui._init = function () {
		...
		if(isEmpty(ui.options.dataId)){
            ...
            var qypchJsonData = $.loadJson($.contextPath +"/sdzycyljyjcxx!getCszzpchGrid.json");
            ui.getItemJQ("QYPCH").combogrid("reload",qypchJsonData.data);
		}else{
			ui.getItemJQ("QYPCH").combogrid("option","readonly",true);
			var $zzpch = ui.getItemValue("QYPCH");
			ui.getItemJQ("QYPCH").combogrid("destroy");
			ui.getItemJQ("QYPCH").textbox();
			ui.getItemJQ("QYPCH").textbox("setValue",$zzpch);
            var formData = $("form", ui.uiForm).form("formData", false);
            if(ui.options.isView){
                $("#chooseImage").removeAttr("onClick");
            }
			if(!isEmpty(formData.TPSC)){
				$.ns("ns"+subffix+"").uploadImage($.contextPath+"/spzstpfj/"+formData.TPSC, ui.options.isView);
			}
		}
		if (ui.options.isView) {
			$("#chooseImage${idSuffix}").hide();
		}
	...
	ui.bindEvent = function (){
		...
		ui.getItemJQ("KSSJ").datepicker("option","onChange", function ( e,data){
			ui.getItemJQ("JSSJ").datepicker("option","minDate",data.value);
		});
	}
	ui.addCallback("setComboGridValue_csgl",function(o){
		if (null == o) return;
...
function _override_grid (ui) {
	...
	ui.clickDelete = function (isLogicalDelete) {
		...
		for(var i=0;i<ids.length;i++){
			if(ui.uiGrid.grid('getRowData',ids[i]).SFJY=='0'){
...
//        view.empty();
		...
		uploadImage : function (tplj,isView){
			var view = $("#view${idSuffix}");
			view.empty();
			var image = new Image();
			image.src = tplj;
			...
			image.onload = function (e) {
				...
				$(div).append(this);
				if (!isView) {
					<%--$(div).append('<span  style="display: none;position: absolute; margin-top: 0px; margin-left: 6px; width: 20px; height: 20px;"><img id="_delete_Btn${idSuffix}" onclick = "$.ns(\'ns'+subffix+'\').deleteImage(\'' + this.src + '\')" style = "height:20px" src = "'+resourceFolder+'/css/images/trace-image/trash_bin.png"></span>');--%>
				}
...
//            div.style.marginTop = "10px";
		...
		deleteImage : function (src) {
			...
			$.ajax({
				type: "POST",
				url: $.contextPath + '/sdzyccscggl!deleteTp.json',
				data: {'id': hUI.options.dataId},
				dataType: "json",
				success: function (data) {
					var view = $("#view${idSuffix}");
					hUI.setFormData({"TPSC":""});
					view.empty();
					$("#imageUpload${idSuffix}").val("");
	...
	function  initTPdiv() {//
		var html = "<div style=\"margin-bottom: 50px;margin-left:55px\">"+
				...
				"<label class=\"app-input-label\" style=\"width:20%;\">图片：</label>"+
