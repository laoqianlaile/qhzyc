 ...
 * @param scfg
 */
function MT_search_area(that, layout, tableId, scfg) {
    ...
    if (1 == scfg.area) {
        ...
        sLayout.collapse();
        that.sform = MT_b_search(that, sLayout, scfg, tableId);
    } else if (2 == scfg.area) {
        sLayout.setHeight(210);
        sLayout.collapse();
        MT_g_search(that, sLayout, tableId);
    } else if (3 == scfg.area) {
        ...
        that.sform = MT_b_search(that, tabbar.cells("grid$search$b"), scfg, tableId);
        MT_g_search(that, tabbar.cells("grid$search$g"), tableId);
        // 默认为基本检索
 ...
 * @param tableId
 */
function MT_g_search(that, sLayout, tableId) {
	...
	var formJson = [
	    {type: "fieldset",  name: "area_1", label: "高级检索", width:300, list:[
	        {type: "hidden", name: "defineConditionId"},
    ...
    gsForm.attachEvent("onChange", function(id, value) {
        if ("defineCondition" == id) {
            gsForm.setItemValue("defineConditionId", value);
            if ("-1" == value.trim())
                return;
	...
	gsForm.attachEvent("onButtonClick", function(id) {
        if (id == "add") {
            ...
            var optVal = con.brakets.left + conCombo.getSelectedValue() + CFG_oc_split + colCombo.getSelectedValue()
                    + CFG_cv_split + val + con.brakets.right;
            var optTxt = con.brakets.left + colCombo.getSelectedText() + con.space + conCombo.getSelectedText()
                    + con.space + val + con.brakets.right;
            if (len > 0) {
            ...
            var multisel = gsForm.getSelect("conditionlist");
            if (-1 === operationMessage(multisel))
                return;
    ...
    function conditionMakeup() {
        ...
        val = idx > 0 ? con.code.and + con.brakets.left + val + con.brakets.right : con.brakets.left + val
                + con.brakets.right;
        txt = idx > 0 ? con.name.and + con.brakets.left + txt + con.brakets.right : con.brakets.left + txt
                + con.brakets.right;
    ...
    function search() {
        ...
        that[pGId].MT_DATA_seach_type = MT_common.S_GREAT;
        that[pGId].MT_DATA_g_seach = filter;
    ...
    function defineSave() {
        ...
        var id = gsForm.getItemValue("defineConditionId");
        var name = gsForm.getCombo("defineCondition").getComboText();
        if ((isEmpty(id) || "-1" == id) && (isEmpty(name) || "请选择或输入检索条件" == name.trim())) {
            dhtmlx.alert("请在检索条件下拉框中输入检索条件名称再保存！");
            return;
        }
        var isSave = (isEmpty(id) || "-1" == id);
        ...
        dhtmlx.confirm({
            type : "confirm",
            text : (isSave ? "确定<font color=\"red\">新增</font>检索条件吗？" : "确定要<font color=\"red\">修改</font>检索条件(" + name
                    + ")吗？"),
            ok : "确定",
            cancel : "取消",
            callback : function(flag) {
                ...
                var url = CFG_app_uri + "/app-search-condition!save.json?tableId=" + tableId + "&moduleId="
                        + that.moduleId + "&condition=" + condition + "&name=" + name;
                if (!isSave)
                    url += "&id=" + id;
                dhtmlxAjax.get(encodeURI(addTimestamp(url)), function(loader) {
                    var entity = eval("(" + loader.xmlDoc.responseText + ")");
                    if (entity.id) {
                        if (isSave) {
                            gsForm.getCombo("defineCondition").addOption([{
                                value : entity.id,
                                text : entity.name,
                                selected : true
                            }]);
                            ...
                            gsForm.setItemValue("defineConditionId", entity.id);
                            defOpts.push({
                                value : entity.id,
                                text : entity.name,
                                prop1 : entity.condition
                            });
                        } else {
                            updateDefOpts(entity);
                            gsForm.getCombo("defineCondition").clearAll();
                            gsForm.getCombo("defineCondition").addOption(defOpts);
                        }
            ...
            callback : function(flag) {
                ...
                dhtmlxAjax.get(encodeURI(addTimestamp(url)), function(loader) {
                    ...
                    if (rlt.success) {
                        ...
                        gsForm.setItemValue("defineCondition", "-1");
                        var multisel = gsForm.getSelect("conditionlist");
                        multisel.options.length = 0;
                        deleteDefOpts(id);
 ...
 * @returns
 */
function MT_b_search(that, sbLayout, scfg, tableId) {
    var pGId = MT_P_GridId(tableId);
