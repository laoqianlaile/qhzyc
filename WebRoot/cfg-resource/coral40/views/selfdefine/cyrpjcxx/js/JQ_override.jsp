...
function _override_grid (ui) {
	...
	ui._init = function(){
		//加载菜品名称
		var spmcData = $.loadJson($.contextPath + "/cyrpjcxx!getAllSpmc.json");
    ...
    function showDiv(ids) {
        ...
        showDiv.dialog({
            ...
            onOpen : function() {
                ...
                var jqShowGrid = $("<div  class=\"coral-adjusted\" style='height:373px'></div>").appendTo(showDiv);
                //加载表单数据
				...
				var result = _window("readAllInfo");
					...
					if(res.status == "true" || res.status == true){
						...
						for(var i = 0; i<records.length;i++){
							...
							jQuery.ajax({
								type:'post',
								url:$.contextPath +'/cyrpjcxx!add.json',
								data:{
									//"JCRQ":jcrq,
						...
						showDiv.dialog({
							...
							onOpen : function() {
								//表单自适应
								$.coral.adjusted(showDiv);
								var jqShowGrid = $("<div  class=\"coral-adjusted\"></div>").appendTo(showDiv);
								//加载表单数据
								new $.config.cgrid(_options, jqShowGrid);
							},
							onClose : function () {
	...
	function setFormData(barCode){
		...
		var ids="";
		if(sysName.toUpperCase() === "PR" || sysName.toUpperCase() === "TZ" ){
			ids = $.loadJson($.contextPath + "/cyrpjcxx!barCodeSave.json?barCode=" + barCode);
		}
		if (ids.length == 0) {
			CFG_message("请输入有效的条形码!", "warning");
			return;
