...
//                var detailGrid = ui.getDetailGrid();
            ...
            var ggfsIndex = $.inArray("灌溉方式", setting.colNames);
            setting.colModel[ggfsIndex].formatter = 'combobox';
            setting.colModel[ggfsIndex].revertCode = true;//存编码
            setting.colModel[ggfsIndex].formatoptions = {//灌溉方式
                'emptyText': '请选择',
                'required': true,
                'data': ggfs,
                'readonly' : isReadonly
            ...
            var sylxIndex = $.inArray("水源类型", setting.colNames);
            setting.colModel[sylxIndex].formatter = 'combobox';
            setting.colModel[sylxIndex].formatoptions = {//水源类型
                'emptyText': '请选择',
                'required': true,
                'data': sylx,
                'readonly' : isReadonly
            ...
            var sydjIndex = $.inArray("水源等级", setting.colNames);
            setting.colModel[sydjIndex].formatter = 'combobox';
            setting.colModel[sydjIndex].formatoptions = {//水源等级
                'emptyText': '请选择',
                'required': true,
                'data': sydj,
                'readonly' : isReadonly
            ...
            $.confirm("当前记录将从列表移除，确定吗？", function (sure) {
                if (sure) {
                    ...
                    if (id.substr(0, 5) == "temp_") {
                        ...
                        var delStatus = $.loadJson($.contextPath + '/zzgg!deleteGg.json?dataId=' + id);
                    ...
                    for (var i in detailGridRowData) {
                        if (nsxbh == detailGridRowData[i].NSZYXBH) {
                            if (detailGridRowData[i].ID.substr(0,5) == "temp_") {
                                ...
                                var delStatus = $.loadJson($.contextPath + '/zzgg!deleteGgTrp.json?dataId=' + detailGridRowData[i].ID);
        ...
        ui.beforeInitGrid = function (setting) {
            ...
            var trpxx = $.loadJson($.contextPath + '/zzbzfa!getTrpxx.json?lx=NJJ');
            var trpTymArray = [];
            for (var i in trpxx) {
                trpTymArray.push({'text':trpxx[i].NJJMC,'value':trpxx[i].NJJBH});
            }
            //投入品通用名
            ...
            var zyxbhArray = [];
            if(_faid != false && _faid != ""){
                zyxbhArray = $.loadJson($.contextPath + "/zzfalb!getZyxbh.json?id=" + _faid + "&tablename=T_ZZ_GG");
            }
            ...
            $.confirm("当前记录将从列表移除，确定吗？", function(sure) {
                if (sure) {
                    if(id.substr(0,5)=="temp_") {
                        ...
                        var delStatus = $.loadJson($.contextPath + '/zzgg!deleteGgTrp.json?dataId='+id);
                        if(delStatus == "SUCCESS") _this.uiGrid.grid("delRowData", id);
                        else CFG_message("删除失败", "error");
                    }
                }
            ...
            if (isInitOp) {
                ...
                $.each($(masterGrid.uiGrid).grid("getDataIDs"),function(e,data){
                    ...
                    if($isStart.combobox("getValue") == "1"){
                        ...
                        $(masterGrid.uiGrid).grid("setCell",data, "IS_START1", "是");
                    }
                });
                var rowData = masterGrid.getRowData();
