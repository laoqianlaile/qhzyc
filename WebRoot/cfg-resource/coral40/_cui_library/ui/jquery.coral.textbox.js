...
( function ( $, undefined ) {
	...
	$.component ( "coral.textbox", $.coral.control, {
		version: $.coral.version,
		castProperties : ["dataCustom","title","buttons", "buttonOptions","triggers"],
		options: {		
			swfPath: $.coral.contextPath+'/jquery-cui/resource/uploadify.swf',
			hasTips: false,
			showStar: true,
			showClose: false,
			dataCustom: null,
		...
		_create: function() {
			...
			if ("hidden" !== this.element.attr("type") && "file" !== this.element.attr("type") && "password" !== this.element.attr("type") && "text" !== this.element.attr("type") && "INPUT" == this.element[0].tagName.toUpperCase()) {
		...
		_initElement: function () {
			...
			if ( this.options.showClose ) {
				this.uiClose = $("<span class='coral-textbox-close icon icon-close2'></span>");
				this.elementBorder.append(this.uiClose);
				this.uiClose.css( "right", this.uiDialogButtonPanel?this.uiDialogButtonPanel.outerWidth():0 );
			}
		...
		_updateTitle: function() {
			...
			/*if ( opts.title ) {
				that.element.attr( "title", opts.title );
			} else if ( typeof that.options.hasTips == "boolean" && that.options.hasTips ) {
				that.element.attr( "title", that.element.val( ) );		
			} else if ( !opts.hasTips ) {
				that.element.attr( "title", "" );
			}  */
		},
		/**
		...
		_bindEvents: function () {
			...
			this._on( {
				...
				"blur": function( event ) {
					...
					//失去焦点时，更新数据到form
					var coralName = $( that.element ).attr( "name" ),
						coralValue = $( that.element ).val();
						//coralData = {};
					/*coralData[coralName] = coralValue;	
					var $form = $( that.element ).parents( "form:first[class*='coral-form-default']" );
					if ( $form.length > 0) {
						$form.form( "updateCoralData", coralData );
					}*/
					if ( "" == this.element.val() ) {
						this._showPlaceholder();
					}
				...
				"keydown": function( event ) {
					...
					if ( false == this._trigger( "onKeyDown", event, [{'value':this.valuebox.val(), 'text':this.element.val(), 'id': this.options.id }] ) ) {
						return false;
					}
				...
				"keypress": function( event ) {
					...
					if ( false === this._trigger( "onKeyPress", event, [{'value':this.valuebox.val(), 'text':this.element.val()}] ) ) {
						return false;
					}
					//key enter and trigger event callback 
					if ( event.keyCode == 13 && options.onEnter ) {
						this.setValue( this.element.val(), true );
				...
				"keyup": function( event ) {
					...
					if ( false === this._trigger( "onKeyUp", event, [{'value':this.valuebox.val(), 'text':this.element.val(), 'id': this.options.id }] ) ) {
						return false;
					}
					this._keyup( event );	
				},
		...
		_setOption: function ( key, value ) {
			...
			if ( key === "readonly" ) {
				if ( typeof value == "boolean" && value ) {		
					$.validate.hideErrors( this.component() );			
					this._setReadonly();
		...
		//设置值
		setValue: function ( value, onChange ) {
			var text = value,nData;
			var _fn1 = $.coral.toFunction(this.options.formatter);
			var _fn2 = $.coral.toFunction(this.options.unformatter);
			if($.isFunction(_fn1)){
				text = _fn1.apply(this.options.element,[{'value':value}]);
				value = _fn2.apply(this.options.element,[{'value':value}]);
			}
			if ( this.element.is("textarea") ) {
				this.setText( text );
			...
			if ( onChange && this.previous != value ) {
