package com.ces.component.qhpfshxx.action;

import com.ces.component.qhpfshxx.dao.QhpfshxxDao;
import com.ces.component.qhpfshxx.service.QhpfshxxService;
...
public class QhpfshxxController extends TraceShowModuleDefineServiceDaoController<StringIDEntity, QhpfshxxService, QhpfshxxDao> {
    ...
    public  void getShxxId(){this.setReturnData(getService().getShxxId());}
...
//        String id  = getService().save("T_SDZYC_JDXX" ,newMap ,null);
        ...
        if (!StringUtil.isEmpty(id)) {//数据保存成功执行文件上传操作
            if (null != this.imageUpload) {
                ...
                try {
                    ...
                    //文件上传成功进行数据保存
                    newMap.put("ID", id);
                    newMap.put("YYZZ", fileNameWithStamp);
                    getService().save("T_QH_PFSHXX", newMap, null);
                } catch (IOException e) {
                    //上传文件失败删除保存数据和图片
    ...
    public void deleteTp(){
        ...
        String cdzm = String.valueOf(dataMap.get("YYZZ"));
