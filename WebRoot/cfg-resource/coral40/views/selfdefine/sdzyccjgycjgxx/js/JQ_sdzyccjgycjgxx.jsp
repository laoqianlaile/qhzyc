...
window[CFG_actionName(subffix)] = function (ui) {
	...
	return jQuery.contextPath + "/sdzyccjgycjgxx";
};
...
function _override_form (ui) {
	ui._init = function(){
		hUI= ui;
		//初始化图片删除区域
		var tpscDiv = ui.getItemBorderJQ("tpsc");
		tpscDiv.empty();
		tpscDiv.html(initTPdiv());
        //判断为详细
        if(ui.options.isView){
            $("#chooseImage${idSuffix}").attr("disabled",true);
        }
		if(isEmpty(ui.options.dataId)){
			...
			ui.getItemJQ("LLDH").combogrid("destroy");
			ui.getItemJQ("LLDH").textbox();
			ui.getItemJQ("LLDH").textbox("option","readonly",true);
			ui.setFormData({LLDH:fromData.LLDH});
			$.ns("ns"+subffix+"").uploadImage($.contextPath+"/spzstpfj/"+fromData.XCTP);
		}
		ui.getItemJQ("JGFZR").combogrid("reload",$.contextPath+"/zzgzryda!getGzrydaGrid.json?dwlx=CJGQY");
	};
	//根据领料单号限制批次号
	ui.bindEvent  = function(){
		var lldh = ui.getItemJQ("LLDH");
		lldh.combogrid("option","onChange",function(e,data){
			var lldhValue = ui.getItemValue("LLDH");
			if(lldhValue!=null){
				var jsonData = $.loadJson($.contextPath +"/cjgylllxxxz!searchLldhGridData.json?lldh="+lldhValue);
			}
		 ui.getItemJQ("YLPCH").combogrid("reload", jsonData);
			//获取联动数据
			var ylpch= ui.getItemJQ("YLPCH");
			ylpch.combogrid("option","onChange",function(e,data){
				var rowData =ylpch.combogrid("grid").grid("getRowData",data.value);
				ui.setFormData({YLPCH:rowData.PCH,YLMC:rowData.LLMC,YLDM:rowData.YCDM,JGZZL:rowData.LLZZL,QYPCH:rowData.QYPCH});
			});

		});
	}
	ui.addCallback("setComboGridValue_jgfzr",function(o){
		if( null == o) return;
		var rowData = o.rowData;
		if( null == rowData) return ;
		ui.setFormData({JGFZR:rowData.XM});
	})
	ui.addOutputValue("setComboGridValue_jgfzr", function (o){
	...
	ui.addCallback("setComboGridValue_jgcj",function(o){
		if( null == o) return;
		var rowData = o.rowData;
		if( null == rowData) return ;
		ui.setFormData({JGCJ:rowData.CJMC});
	})
	ui.addCallback("setComboGridValue_jggy",function(o){
		if( null == o) return;
		var rowData = o.rowData;
		if( null == rowData) return ;
		ui.setFormData({JGGY:rowData.GYMC});
	})
...
//            div.style.marginTop = "10px";
	...
	deleteImage : function (src) {
		...
		$.ajax({
			type: "POST",
			url: $.contextPath + '/sdzyccjgycjgxx!deleteTp.json',
			data: {'id': hUI.options.dataId},
			dataType: "json",
			success: function (data) {
				var view = $("#view${idSuffix}");
				hUI.setFormData({"XCTP":""});
				view.empty();
				$("#imageUpload${idSuffix}").val("");
...
function  initTPdiv() {//
	var html = "<div style=\"margin-bottom: 50px;margin-left:55px\">"+
		"<div class=\"fillwidth colspan2 clearfix\">"+
				...
				"<label class=\"app-input-label\" style=\"width:20%;\">现场图片：</label>"+
