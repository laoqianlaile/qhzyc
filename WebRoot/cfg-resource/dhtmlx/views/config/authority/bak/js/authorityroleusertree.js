var menuItemId, type, classification;
/**
 * 权限定义树初始化
 * @param layoutA
 * @param layoutB
...
function initAuthorityTree(layoutA, layoutB) {
    var moduelUrl = contextPath + "/authority/authority";
    ...
    tree.setXMLAutoLoading(moduelUrl + "!tree.json?E_model_name=tree&F_in=text,child&P_UD=type");
    tree.loadJSONObject(treeJson);
    // 点击节点刷新右边列表
    tree.attachEvent("onClick", function(id) {
        if (id == "-A" || id == "-R" || id == "-U" || id.indexOf("O_")==0) {
