	...
	//method of event handler
		...
		_apply: function(fun, param, defaultValue) {			
			...
			/*if ((typeof fun) == "function") {
				return fun.apply(coralTree, param?param:[]);
			}
			//传递回调方法时，判断是否为字符串类型，如果是，则使用window[fun]调用
			if((typeof fun) == "string"){
				try {
					var f = eval("(" + fun + ")");
					if ($.isFunction( f )) {
						return f.apply(coralTree, param?param:[]);
					}
				} catch (e) {}
			}
			return defaultValue;*/
			...
			$.ajax({
				...
				success: function(msg) {					
					...
					if (newNodes && newNodes !== "") {
						...
						$.tree._addNodes(setting, node, [], !!isSilent);
					}
					if ( opts.onLoad ) {
	...
	$.fn.tree = function(options){
		...
		this.each(function() {
			...
			if ( isMethodCall ) {
				...
				//if(args.length == 2){
					//var _options2 = $.extend( {}, $.fn['tree'].defaults || {}, options );
				$.coral.tree.init( $(this), _options , node );
				//	return;
				//}
				//没有参数时，调用初始化init方法
				/*$.data( this, "treeOptions", _options);
				$.coral.tree.init($(this), _options, _options.data);*/
			}
		});
		return returnValue;
	};
