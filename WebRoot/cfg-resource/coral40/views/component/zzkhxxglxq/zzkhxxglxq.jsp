...
<div id="max${idSuffix}" class="fill" >
	...
	<div id="ddFormDiv${idSuffix}" style="margin-left: 20px;overflow: auto;height: 452px; ">
		<form id="khForm${idSuffix}" action="${basePath}zzkhxxg!saveKhxx.json"
			...
			<div class="fillwidth colspan3 clearfix">
				...
				<div class="app-inputdiv4" style ="height:32px;display: none">
					<input id="LSXZQHBM${idSuffix}" name="LSXZQHBM"  class="coralui-textbox" ></input>
				</div>
				<!------------------ 第四排结束---------------->
				<div class="app-inputdiv12">
					<label class="app-input-label" >地址：</label>
					<input id="DZ${idSuffix}" name="DZ"  data-options="required:true"  class="coralui-textbox"></input>
				</div>
				...
				<div class="app-inputdiv4">
					...
					<input id="MM${idSuffix}" type="password" name="MM" data-options="readonly:true" class="coralui-textbox"/>
				</div>
				<div class="app-inputdiv4">
				...
				<div class="app-inputdiv4">
					...
					<button  class='ctrl-toolbar-element ctrl-init ctrl-init-button coral-button coral-component coral-state-default coral-corner-all coral-button-text-only coral-toolbar-item-component'
							type='button' onclick="$('#tp${idSuffix}').click() " style="position:absolute;display:none;">
				...
				<div class="app-inputdiv4">
					...
					<div id=buttonlogo${idSuffix}" style="margin-left: 0px">
						<button style="margin-left: 0px;display:none;" id="chooseImage${idSuffix}"
	...
	<div class="toolbarsnav clearfix" style="display: none">
		...
		<div class='homeSpan' style="margin-top: -23px;display: none" ><div><div style='margin-left:20px;width: 100px;' id="nva${idSuffix}"> -客户门店信息 </div></div></div>
	</div>
	<%--<ces:grid id="gridId${idSuffix}"  forceFit="false" fitStyle="fill" datatype="local" shrinkToFit="false" >
		<ces:gridCols>
			<ces:gridCol name="ID" key="true" hidden="true"  width="100">ID</ces:gridCol>
			<ces:gridCol name="TP"  hidden="true"  width="100">TP</ces:gridCol>
			<ces:gridCol name="PID"  hidden="true" width="80">PID</ces:gridCol>
			<ces:gridCol name="MDMC" formatter="text" formatoptions="required: true,readonly:true" width="200px">门店名称</ces:gridCol>
			<ces:gridCol name="MDDZ" formatter="text" formatoptions="required: true,readonly:true" width="300px">门店地址</ces:gridCol>
			<ces:gridCol name="MDLXR" formatter="text" formatoptions="required: true,readonly:true" width="200px">门店联系人</ces:gridCol>
			<ces:gridCol name="YHM"  width="140">用户名</ces:gridCol>
			<ces:gridCol name="MM" width="120">密码</ces:gridCol>
			<ces:gridCol name="MDLXDH"  formatter="text" formatoptions="required: true,validType:'mobile',readonly:true" width="200px">门店联系电话</ces:gridCol>
			<ces:gridCol name="MDLXRSJ"  formatter="text" formatoptions="required: true,validType:'mobile',readonly:true" width="200px">门店联系人手机</ces:gridCol>
			<ces:gridCol name="JD" formatter="text" formatoptions="required: true,validType:'float',readonly:true" width="100px">经度</ces:gridCol>
			<ces:gridCol name="WD"  formatter="text" formatoptions="required: true,validType:'float',readonly:true" width="100px">纬度</ces:gridCol>
			<ces:gridCol name="SSDQ" formatter="combobox" formatoptions="required: true,readonly:true"
						 width="220">所属地区</ces:gridCol>
			<ces:gridCol name="SSDQBM" formatter="text" formatoptions="required: true,readonly:true"
						 width="150" >所属地区编码</ces:gridCol>
			<ces:gridCol name="CZ" editable="false" formatter="toolbar"
						 formatoptions="isOverflow:false,onClick:$.ns('namespaceId${idSuffix}').toolbarClick3,
						             data:[
						             {'label': '查看图片', 'id':'viewBtn', 'disabled': 'false','type': 'button'}
						             ]" width="230px">操作</ces:gridCol>
		</ces:gridCols>
	</ces:grid>--%>
...
//				}
		...
		var configInfo = CFG_initConfigInfo({
			...
			'bodyOnLoad' : function(configInfo) {
				...
				$("#LSXZQH${idSuffix}").combogrid("option","onChange",function(e,data){
					$("#LSXZQH${idSuffix}").combogrid("setValue",data.text);
				...
				if(isEmpty(inputParamId)){//如果是新增操作在设置临时ID
					...
					$("#nva${idSuffix}").html("-客户信息-详情");
					$("#khForm${idSuffix}").form("setReadOnly", true);
	...
	$("#khForm${idSuffix}").submit(function (){
		if($("#YHM${idSuffix}").textbox("getValue").length > 0){
			if($("#MM${idSuffix}").textbox("getValue").length == 0){
				<%--$("#MM${idSuffix}").textbox("setValue","000000");--%>
		...
		if(isValid){//
			$.ajax({
				...
				success: function (data) {
					...
					//门店列表数据初始化
					$grid.grid("option", "datatype", "json");
					$grid.grid("option","url", $.contextPath + "/zzkhxxgllb!searchKhmdxx.json?pid="+temId);
					$grid.grid("reload");
	...
	function submitForm(form) {
		...
		$.ajax({
			...
			success: function (data) {
				gData.TP = data.TP;
				alert(data.TP+" - - --- " + gData.ID);
				$grid.grid("setRowData",gData.ID, gData);
				//$grid.grid("setRowData",rowId, dataAdded);
				CFG_message("操作成功！", "success");
			},
			error: function () {
