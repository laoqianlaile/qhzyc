...
<script type="text/javascript">
    ...
    (function(subffix) {
        ...
        window[CFG_actionName(subffix)] = function (ui) {
            ...
            return jQuery.contextPath + "/jjgjyjcdwjsc";
        };
...
//            initToolbar();
            hUI = ui;
            ui._init = function () {
                ui.getItemJQ("JYRY").combogrid("reload",$.contextPath+"/zzgzryda!getGzrydaGrid.json?dwlx=JJGQY");
                if( ui.options.number == 1 ){
                    if (isEmpty(ui.options.dataId)) {
                        var checkType = ui.assembleData.params.selfParams[0].value;
                        var jyxx="企业自检";
                        var qybm = "<%=qybm%>";
                        if( "gov" == checkType){
                            jyxx = "政府飞检";
                        }else if( "cons" == checkType){
                            jyxx = "委托待检";
                            ui.getItemJQ("PCH").combogrid("option","url",$.contextPath +"/sdzycjjgypjyjcxx!getycjy.json");
                            ui.getItemJQ("PCH").combogrid("reload");
                        }else{
                            ui.getItemJQ("PCH").combogrid("option","url",$.contextPath +"/sdzycjjgypjyjcxx!getycjy.json");
                            ui.getItemJQ("PCH").combogrid("reload");
                        }
                        ui.setFormData({QYBM: qybm,JYXX: jyxx});
                    }else{
            ...
            ui.bindEvent = function () {
                if(ui.options.number ==1 ){
                    ...
                    pch.combogrid("option", "onChange", function (e, data) {
                        ...
                        ui.setFormData({PCH: rowData.SCPCH, YPMC: rowData.YPMC,  QYPCH:rowData.QYSCPCH,YCDM:rowData.YCDM});
                    });
                }
            }
        ...
        function _override_grid (ui) {
            ...
            ui.toFormData = function () {
                ...
                var fDataArr = [], i = 0, len = rData.length,
                        tRow, sRow, prop;
                for (; i < len; i++) {
                    sRow = rData[i];
                    tRow = {};
