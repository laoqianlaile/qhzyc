...
window[CFG_actionName(subffix)] = function (ui) {
	...
	return jQuery.contextPath + "/sdzyczzzmxx";
};
...
//		var qybm = $.loadJson($.contextPath + "/trace!getQybm.json");
		...
		if (isEmpty(ui.options.dataId)){
			var lxData = $.loadJson($.contextPath+"/sdzyczzzmxx!getLx.json");
			ui.getItemJQ("LX").combobox({
				textField: 'TEXT',
				valueField: 'VALUE',
				required: true
			});
			ui.getItemJQ("LX").combobox("reload",lxData);
		}
		//修改
		if (!isEmpty(ui.options.dataId)){
			var lxData = $.loadJson($.contextPath+"/sdzyczzzmxx!getLx.json");
			...
			ui.getItemJQ("LX").combobox("reload",lxData);
			var lxData = $.loadJson($.contextPath+"/sdzyczzzmxx!getLxById.json?id="+ui.options.dataId);
			ui.getItemJQ("LX").combobox("setValue", lxData.LX);
		}
		//初始化图片删除区域
		var sctpDiv = ui.getItemBorderJQ("sctp");
		sctpDiv.empty();
		...
		if(isEmpty(ui.options.dataId)){
			...
			if(!isEmpty(fromData.TPSC)){
				$.ns("ns"+subffix+"").uploadImage($.contextPath+"/spzstpfj/"+fromData.TPSC, ui.options.isView);
			}
		}
		if (ui.options.isView) {
...
//        var ima_data = $(".thumb").attr("src");
//        if(isEmpty(ima_data)){
//            CFG_message("请添加图片","warning");
//            return false;
//        }
//    }
...
//            div.style.marginTop = "10px";
		...
		deleteImage : function (src) {
			...
			$.ajax({
				type: "POST",
				url: $.contextPath + '/sdzycjdxx!deleteTp.json',
	...
	function  initTPdiv() {//
		var html = "<div style=\"margin-bottom: 50px;margin-left:42px\">"+
