...
function _override_grid (ui) {
	...
	ui._init = function(){
		//加载菜品名称
		var spmcData = $.loadJson($.contextPath + "/ttrpjc!getAllSpmc.json");
				...
				var result = _window("readAllInfo");
					...
					if(res.status == "true" || res.status == true){
						...
						for(var i = 0; i<records.length;i++){
							...
							jQuery.ajax({
								type:'post',
								url:$.contextPath +'/ttrpjc!add.json',
								data:{
									"JCRQ":jcrq,
	...
	function setFormData(barCode){
		...
		if(sysName.toUpperCase() === "PR" || sysName.toUpperCase() === "TZ" ){
			ids = $.loadJson($.contextPath + "/ttrpjc!barCodeSave.json?barCode=" + barCode);
		}
		if (ids.length == 0) {
			CFG_message("请输入有效的条形码!", "warning");
			return;
