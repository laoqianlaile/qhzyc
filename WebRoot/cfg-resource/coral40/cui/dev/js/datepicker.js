...
$.extend(Datepicker.prototype, {
	...
	_optionDatepicker: function(target, key, value) {
		...
		if (inst) {
			...
			if(key=="value"){
				if (value) {
					var format = this.getFormatter(value,this._get(inst,"dateFormat"), this._getFormatConfig(inst));
					var cdate = this.parseDate(format, value);
					var srcDateFormat = this._get(inst,"srcDateFormat");
					if (this._checkDateRange(inst, cdate) === 2){
						value = this.formatDate(srcDateFormat, minDate, this._getFormatConfig(inst));
					}
					if (this._checkDateRange(inst, cdate) === 1) {
						value = this.formatDate(srcDateFormat, maxDate, this._getFormatConfig(inst));
					}
				}
			...
			if( key=="value" ){
				$.datepicker._setStartEnd(inst, oldValue, value);
			}
			/*//防止日期框意外显示
	...
	_doKeyDown: function(event) {
		...
		if ($.datepicker._datepickerShowing) {
			...
			case 13: 
				...
				$.datepicker._setStartEnd(inst, oldValue, datestr);
	...
	_setStartEnd: function(inst, oldValue, value){
		/*var od = this.parseDate($.datepicker._get(inst, "dateFormat"), oldValue),
			nd = this.parseDate($.datepicker._get(inst, "dateFormat"), value);
		od = od?od.getTime() : "";
		nd = nd?nd.getTime() : "";*/
		if (oldValue == value) {
			return;
	...
	_setHiddenInputValue :function(inst) {
		...
		if(this._get(inst,"complete")){
			dateStr=this.formatDate(srcDateFormat, date, this._getFormatConfig(inst));
		}else {
...
$.fn.datepicker = function(options){
	...
	this.each(function() {
		...
		//判断为调用方法
			...
			methodValue = typeof options === "string" ?
					$.datepicker["_" + options + "Datepicker"].
					apply($.datepicker, [this].concat(otherArgs)) :
						$.datepicker._attachDatepicker(this, options);
