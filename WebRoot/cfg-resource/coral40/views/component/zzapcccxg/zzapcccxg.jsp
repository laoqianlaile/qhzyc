...
<script type="text/javascript">
    ...
    $(function () {
        var configInfo = CFG_initConfigInfo({
            ...
            'bodyOnLoad': function (configInfo) {
                ...
                var id = CFG_getInputParamValue(configInfo, 'dataId');
                ...
                var $dkData = $.loadJson($.contextPath + "/zzcc!getDkbhByQybh.json?qybh=" + $("#QYBH${idSuffix}").combobox("getValue"));
                $("#DKBH${idSuffix}").combobox("reload",$dkData);
                var $scdabhData = $.loadJson($.contextPath + "/zzcc!getScdabhByDkbh.json?dkbh=" + $("#DKBH${idSuffix}").combobox("getValue"));
                $("#SCDABH${idSuffix}").combobox("reload",$scdabhData);
                var $pchData = $.loadJson($.contextPath + "/zzcc!getCspchByScdabh.json?scdabh=" + $("#SCDABH${idSuffix}").combobox("getValue"));
                $("#PCH${idSuffix}").combobox("reload",$pchData);
                ...
                $.each($grid.grid("getDataIDs"),function(e,data){
                    ...
                    var $ddbhData = $.loadJson($.contextPath + "/zzcc!getXsddbhByKhbh.json?khbh=" + $khbhCombobox.combobox("getValue")  +"&ddbh=" + ddbhs);
                    $ddbhData.push($rowGridData);
                    $ddbhCombobox.combobox("reload",$ddbhData);
                    $ddbhCombobox.combobox("setValue", $rowGridData.DDBH);
                });

                var cczzl = 0.0;
                $.each($grid.grid("getRowData"),function(e, data){
                    cczzl += parseFloat(data.CCZL);
                });
                var kczl = cczzl + parseFloat($("#KCZL${idSuffix}").textbox("getValue"));
    ...
    $("#pcxxForm${idSuffix}").submit(function () {
        ...
        $.ajax({
            type: 'POST',
            url: $.contextPath + "/zzcc!updatePcccxx.json",
            data: postData,
            dataType: "json",
