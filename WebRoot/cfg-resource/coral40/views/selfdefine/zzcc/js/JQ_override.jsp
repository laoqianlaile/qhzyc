...
//                var detailGrid = ui.getDetailGrid();
                    ...
                    //锄草方式
                    var ccfsIndex = $.inArray("锄草方式",setting.colNames);
                    var ccfs = $.loadJson($.contextPath + "/trace!getDataDictionary.json?lxbm=ZZSCCCFS");
                    setting.colModel[ccfsIndex].formatter = 'combobox';
                    setting.colModel[ccfsIndex].revertCode = true;//存编码
                    setting.colModel[ccfsIndex].formatoptions = {
                        'emptyText': '请选择',
                        'required': true,
                        'data' : ccfs,
                        'readonly' : isReadonly
                    ...
                    if (isView == "VIEW" || isView == "DETAIL") {
                        ...
                        setting.colModel[delIndex].formatoptions = {
                            'required': true,
                            'readonly' : true
                        };
                    }
                    ...
                    $.confirm("当前记录将从列表移除，确定吗？", function (sure) {
                        if (sure) {
                            ...
                            if (id.substr(0, 5) == "temp_") {
                                ...
                                var delStatus = $.loadJson($.contextPath + '/zzcc!deleteCc.json?dataId=' + id);
                            ...
                            for (var i in detailGridRowData) {
                                if (nsxbh == detailGridRowData[i].NSZYXBH) {
                                    if (detailGridRowData[i].ID.substr(0,5) == "temp_") {
                                        ...
                                        var delStatus = $.loadJson($.contextPath + '/zzcc!deleteCcTrp.json?dataId=' + detailGridRowData[i].ID);
...
//                                    ui.uiGrid.grid("setCell", e.data.rowId, "YT", trpxx[i].YT);
                    ...
                    var zyxbhArray = [];
                    if(_faid != false && _faid != ""){
                        zyxbhArray = $.loadJson($.contextPath + "/zzfalb!getZyxbh.json?id=" + _faid + "&tablename=T_ZZ_CC");
                    }
                    ...
                    $.confirm("当前记录将从列表移除，确定吗？", function(sure) {
                        if (sure) {
                            if(id.substr(0,5)=="temp_") {
                                ...
                                var delStatus = $.loadJson($.contextPath + '/zzcc!deleteCcTrp.json?dataId='+id);
                                if(delStatus == "SUCCESS") _this.uiGrid.grid("delRowData", id);
                                else CFG_message("删除失败", "error");
                            }
                        }
