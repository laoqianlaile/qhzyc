...
window[CFG_actionName(subffix)] = function (ui) {
	...
	return jQuery.contextPath + "/qhpfshxx";
};
...
function _override_form (ui) {
	...
	ui._init = function(){
		...
		hUI= ui;
		//初始化图片删除区域
		var wjscDiv = ui.getItemBorderJQ("YYZZ");
		wjscDiv.empty();
		...
		if(isEmpty(dataid)){
			...
			if (!isEmpty(fromData.YYZZ)) {
				$.ns("ns" + subffix + "").uploadImage($.contextPath + "/spzstpfj/" + fromData.YYZZ, ui.options.isView);
			}
		}
...
//});
	function  initTPdiv() {//
		var html = "<div style=\"margin-bottom: 50px;margin-left:55px\">"+
				...
				"<label class=\"app-input-label\" style=\"width:20%;\">营业执照：</label>"+
...
//            div.style.marginTop = "10px";
		...
		deleteImage : function (src) {
			...
			$.ajax({
				type: "POST",
				url: $.contextPath + '/qhpfshxx!deleteTp.json',
				data: {'id': hUI.options.dataId},
				dataType: "json",
				success: function (data) {
					var view = $("#view${idSuffix}");
					hUI.setFormData({"YYZZ":""});
					view.empty();
					$("#imageUpload${idSuffix}").val("");
