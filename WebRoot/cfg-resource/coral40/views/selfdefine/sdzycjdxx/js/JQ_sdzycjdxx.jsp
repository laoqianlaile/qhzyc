...
window[CFG_actionName(subffix)] = function (ui) {
	...
	return jQuery.contextPath + "/sdzycjdxx";
};
...
function _override_form (ui) {
	...
	ui._init = function(){
		...
		hUI= ui;
		//初始化图片删除区域
		var wjscDiv = ui.getItemBorderJQ("wjsc");
		wjscDiv.empty();
		wjscDiv.html(initTPdiv());
		if(isEmpty(dataid)){
			qybm.textbox("setValue",$.loadJson($.contextPath + "/trace!getQybm.json"));
			ui.getItemJQ("FZR").combogrid("reload",$.contextPath+"/zzgzryda!getGzrydaGrid.json?dwlx=ZZQY");
		}else{
			...
			if(!isEmpty(fromData.CDZM)){
				$.ns("ns"+subffix+"").uploadImage($.contextPath+"/spzstpfj/"+fromData.CDZM,ui.options.isView);
			}
		}
		if (ui.options.isView) {
			$("#chooseImage${idSuffix}").hide();
		}
		var fzr = ui.getItemValue("FZR");
		ui.getItemJQ("FZR").combogrid("reload",$.contextPath+"/zzgzryda!getGzrydaGrid.json?dwlx=ZZQY");
		ui.getItemJQ("FZR").combogrid("setValue", fzr);
	}
...
function _override_bsearch (ui) {
	...
	ui._init = function(){
		...
		var button = document.getElementById("FZR_button");
    ...
    ui.addCallback("setComboGridValue_fzr",function(o){
        ...
        ui.setItemValue("FZR", rowData.XM);
    });
...
//            div.style.marginTop = "10px";
	...
	function  initTPdiv() {//
		var html = "<div style=\"margin-bottom: 50px;margin-left:55px\">"+
				...
				"<label class=\"app-input-label\" style=\"width:20%;\">产地证明：</label>"+
