...
//                var detailGrid = ui.getDetailGrid();
            ...
            //施肥时段
            var sfsdIndex = $.inArray("施肥时段",setting.colNames);
            var sfsd = $.loadJson($.contextPath + "/trace!getDataDictionary.json?lxbm=ZZSFSD");
            setting.colModel[sfsdIndex].formatter = 'combobox';
            setting.colModel[sfsdIndex].revertCode = true;//存编码
            setting.colModel[sfsdIndex].formatoptions = {
                'emptyText': '请选择',
                'required': true,
                'data' : sfsd,
                'readonly' : isReadonly
            ...
            $.confirm("当前记录将从列表移除，确定吗？", function (sure) {
                if (sure) {
                    ...
                    if (id.substr(0, 5) == "temp_") {
                        ...
                        var delStatus = $.loadJson($.contextPath + '/zzsffa!deleteSffa.json?dataId=' + id);
                    ...
                    for (var i in detailGridRowData) {
                        if (nsxbh == detailGridRowData[i].NSZYXBH) {
                            if (detailGridRowData[i].ID.substr(0,5) == "temp_") {
                                ...
                                var delStatus = $.loadJson($.contextPath + '/zzsffa!deleteSffaTrp.json?dataId=' + detailGridRowData[i].ID);
        ...
        ui.beforeInitGrid = function (setting) {
            ...
            var trpxx = $.loadJson($.contextPath + '/zzbzfa!getTrpxx.json?lx=FL');
            var trpTymArray = [];
            for (var i in trpxx) {
...
//                            ui.uiGrid.grid("setCell", e.data.rowId, "YT", trpxx[i].YT);
            ...
            var zyxbhArray = [];
            if(_faid != false && _faid != ""){
                zyxbhArray = $.loadJson($.contextPath + "/zzfalb!getZyxbh.json?id=" + _faid + "&tablename=T_ZZ_SF");
            }
            ...
            $.confirm("当前记录将从列表移除，确定吗？", function(sure) {
                if (sure) {
                    if(id.substr(0,5)=="temp_") {
                        ...
                        var delStatus = $.loadJson($.contextPath + '/zzsffa!deleteSffaTrp.json?dataId='+id);
                        if(delStatus == "SUCCESS") _this.uiGrid.grid("delRowData", id);
                        else CFG_message("删除失败", "error");
                    }
                }
