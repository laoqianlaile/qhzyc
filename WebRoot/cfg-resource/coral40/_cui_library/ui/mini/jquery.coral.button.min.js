...
}},refresh:function(){var i=this.element.is("input, button")?this.element.is(":disabled"):this.element.hasClass("coral-button-disabled");if(i!==this.options.disabled){this._setOption("disabled",i)}if(this.type==="radio"){a(this.element[0]).each(function(){if(f(this).is(":checked")){f(this).button("component").addClass("coral-state-active").attr("aria-pressed","true")}else{f(this).button("component").removeClass("coral-state-active").attr("aria-pressed","false")}})}else{if(this.type==="checkbox"){if(this.element.is(":checked")){this.buttonElement.addClass("coral-state-active").attr("aria-pressed","true")}else{this.buttonElement.removeClass("coral-state-active").attr("aria-pressed","false")}}}},_resetButton:function(){if(this.type==="input"){if(this.options.label){this.element.val(this.options.label)}return}var m=this.buttonElement.removeClass(g),k=f("<span></span>",this.document[0]).addClass("coral-button-text").html(this.options.label).appendTo(m.empty()).text(),j=this.options.icons,i=j.primary&&j.secondary,l=[];if(j.primary||j.secondary){if(this.options.text){l.push("coral-button-text-icon"+(i?"s":(j.primary?"-primary":"-secondary")))}if(j.primary){m.prepend("<span class='coral-button-icon-primary icon "+j.primary+"'></span>")}if(j.secondary){m.append("<span class='coral-button-icon-secondary icon "+j.secondary+"'></span>")}if(!this.options.text){l.push(i?"coral-button-icons-only":"coral-button-icon-only");if(!this.hasTitle){m.attr("title",f.trim(k))}}}else{l.push("coral-button-text-only")}if(this.hasTitle){m.attr("title",f.trim(k))}m.addClass(l.join(" "))}});f.component("coral.buttonset",{version:"4.0.1",options:{items:"button, input[type=button], input[type=submit], input[type=reset], input[type=checkbox], input[type=radio], a, :data(coral-button)"},_create:function(){this.element.addClass("coral-buttonset")},_init:function(){this.refresh()},_setOption:function(i,j){if(i==="disabled"){this.buttons.button("option",i,j)}this._super(i,j)},refresh:function(){var j=this.element.css("direction")==="rtl",i=this.element.find(this.options.items),k=i.filter(":coral-button");
