...
//Editing
	editButtonsPos: function(rowid,inlineBtn){
		var $t = this,ids,ind,height,gridView;
		ids = $($t.element).grid("getDataIDs");
		ind = $($t.element).grid("getInd",rowid,true);
		gridView = $t.element.find(".coral-grid-view");
		height = $t.element.find(".coral-grid-btable").height();
		if((height + 20 > gridView.height()) && rowid == ids[ids.length-1]){
			inlineBtn.position({
