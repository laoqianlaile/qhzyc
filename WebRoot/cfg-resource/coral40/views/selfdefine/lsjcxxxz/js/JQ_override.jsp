...
function _override_form (ui) {
		...
		ui.clickSecondDev = function(id){
			...
			if(id==$.config.preButtonId+"readCard"){
				if(isNewSwt){
					try{
						//错误的调用
						var readResult = _card.read();
					}catch(e){
						//显示异常的详细信息
						$.alert("读卡出错，请检查设备！");
						return false;
					}
					setFormData(readResult);
				}else{
					$.alert("请在程序中操作");
				}
			}
    ...
    function setPfsxx(jyzbm, barCode) {//根据读码信息获取本市场内该经营者信息
        ...
        if ("FATAL" == jyzxx.result) {
            ...
            var id = jyzxx.ID;
            var createJyz = $.loadJson($.contextPath + '/jyzxx!createJyz.json?id=' + id + "&xltx=LS&barCode=" + barCode);
            ui.setFormData({
                PFSMC: jyzxx.JYZMC,
                PFSBM: jyzxx.JYZBM
            });
...
//		}
		...
		if("ZZ" === prefix){
			...
			if ("error" === jsonData.result) {
                ...
                //更新主表理货重量 金额
                var rowData = detailGrid.getRowData(), zl, je, js, zzl = 0, zje = 0, zjs = 0;
                for (var i = 0, len = rowData.length; i < len; i++) {
                    zl = rowData[i].ZL;
                    je = rowData[i].JE;
                    js = rowData[i].JS;
