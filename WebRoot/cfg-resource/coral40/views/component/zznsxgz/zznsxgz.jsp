...
<%
    ...
    String nsxlx = request.getParameter("P_columns");
    request.setAttribute("nsxlx", nsxlx);
%>
<style>
...
<div id="maxDiv${idSuffix}" class="fill">
    <ces:layout id="layout${idSuffix}" name="" style="width:800px;height:700px;" fit="true">
        <ces:layoutRegion region="north" split="true" style="height:220px;" title="农事项跟踪">
            <div class="fillwidth colspan2 clearfix" style="margin-top: 10px">
                ...
                <div class="app-inputdiv6">
                    <label class="app-input-label">农事项结束时间:</label>
                ...
                <div class="app-inputdiv6">
                    ...
                    <ces:radiolist id="rwzt${idSuffix}" name="radiolist" value="1"
                                   column="3"
        ...
        <ces:layoutRegion region="center" split="true">
            <ces:toolbar onClick="$.ns('namespaceId${idSuffix}').toolbarClick2"
                         data="['->',{'label': '修改', 'id':'edit','cls':'', 'type': 'button'},{'label': '删除', 'id':'delete','cls':'', 'type': 'button'},{'label': '评价', 'id':'judge','cls':'', 'type': 'button'}]">
            </ces:toolbar>
            <div style="margin-top: -40px"><div class='homeSpan' style="height:23px"><div style='margin-left:25px;width: 170px;font-size:16px' id="nva${idSuffix}"> - 农事项任务跟踪管理 </div></div></div>
            <div id="nsxgzGridDiv${idSuffix}" style="height:632px">
                <ces:grid rowNum="10" multiselect='true' id="nsxgzGrid${idSuffix}" height="auto"
                    ...
                    <ces:gridCols>
                        ...
                        <ces:gridCol name="PJ" formatter="$.ns('namespaceId${idSuffix}').setPj"
                                     width="100">评价</ces:gridCol>
                    </ces:gridCols>
    ...
    $.extend($.ns("namespaceId${idSuffix}"), {
        toolbarClick1: function (e, data) {
            if (data.id == 'search') {
                ...
                $("#rwzt${idSuffix}").radiolist("setValue", "1");
                $("#pjzt${idSuffix}").radiolist("setValue", "1");
            }
        },
        toolbarClick2: function (e, data) {
            if (data.id == 'edit') {
                ...
                for (var id in selArrId) {
                    var rowData = $("#nsxgzGrid${idSuffix}").grid("getRowData",selArrId[id]);
                    if (rowData.ZT=="未完成") {
                        CFG_message("农事项未完成,不可评价", "warning");
                        return;
                    }
                    ids.push(rowData.ID);
                }
        ...
        showEditDiv: function (_id, _fzr, _ygsj,_lx) {
            ...
            content += "</div>";
            content += "<div class='app-inputdiv6'>";
            content += "<label class='app-input-label'>预计作业时间:</label>";
            content += "<input id = 'ygsjEdit${idSuffix}' width='50%' />";
            content += "</div>";
            content += "</div>";
            content += "</div>";
        ...
        showJudgeDiv: function (_ids) {
            ...
            content += "<div id='showDialog'>";
                ...
                content += "<div class = 'fillwidth colspan1 clearfix' style='margin-top:20px'>";
                    content += "<div class='app-inputdiv12'>";
                        content += "<label class='app-input-label'>评价:</label>";
                        content += "<input id='judge${idSuffix}'style='#F08300'/>";
                    content += "</div>";
                content += "</div>";
            content += "</div>";
            var showDiv = $(content).appendTo(jqGlobal);
            showDiv.dialog({
                ...
                onOpen: function () {
                    ...
                    function judge() {
                        var pj = $("#judge${idSuffix}").combobox("getValue");
                        if (pj == "") {
                            CFG_message("请选择评价等级", "warning");
                            return;
                        }
                        $.ajax({
                            dataType: "json",
                            url: $.contextPath + '/zznsxgz!judgeNsx.json',
                            data: {ids: JSON.stringify(_ids),pj: pj},
                            success: function (data) {
                                CFG_message("评价成功", "success");
                                showDiv.remove();
                                $.ns("namespaceId${idSuffix}").loadGridData();
                            },
                            error: function (data) {
    ...
    $(function () {
        var configInfo = CFG_initConfigInfo({
            ...
            'bodyOnLoad': function (configInfo) {
                ...
                var queryInfos = "${nsxlx}";
                var queryInfo = queryInfos.split(",");
                if(queryInfo.length == 2){
                    $("#rwzt${idSuffix}").radiolist("setValue",queryInfo[0]);
                    $("#nsxlx${idSuffix}").combobox("setValue",queryInfo[1]);
                    $.ns("namespaceId${idSuffix}").setUrl();
                    $.ns("namespaceId${idSuffix}").loadGridData();
                }
