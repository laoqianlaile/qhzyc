                    ...
                    $.confirm("当前记录将从列表移除，确定吗？", function (sure) {
                        if (sure) {
                            ...
                            if (id.substr(0, 5) == "temp_") {
                                ...
                                var delStatus = $.loadJson($.contextPath + '/zzqt!deleteQt.json?dataId=' + id);
                            ...
                            for (var i in detailGridRowData) {
                                if (nsxbh == detailGridRowData[i].NSZYXBH) {
                                    if (detailGridRowData[i].ID.substr(0,5) == "temp_") {
                                        ...
                                        var delStatus = $.loadJson($.contextPath + '/zzqt!deleteQtTrp.json?dataId=' + detailGridRowData[i].ID);
...
//                                    ui.uiGrid.grid("setCell", e.data.rowId, "YT", trpxx[i].YT);
                    ...
                    var zyxbhArray = [];
                    if(_faid != false && _faid != ""){
                        zyxbhArray = $.loadJson($.contextPath + "/zzfalb!getZyxbh.json?id=" + _faid + "&tablename=T_ZZ_QT");
                    }
                    ...
                    $.confirm("当前记录将从列表移除，确定吗？", function(sure) {
                        if (sure) {
                            if(id.substr(0,5)=="temp_") {
                                ...
                                var delStatus = $.loadJson($.contextPath + '/zzqt!deleteQtTrp.json?dataId='+id);
                                if(delStatus == "SUCCESS") _this.uiGrid.grid("delRowData", id);
                                else CFG_message("删除失败", "error");
                            }
                        }
