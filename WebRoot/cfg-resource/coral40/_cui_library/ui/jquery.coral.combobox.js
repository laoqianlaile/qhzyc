...
$.component( "coral.combobox", $.coral.combo, {
	...
	_create : function() {
		...
		this.uiCombo.panel.unbind().bind('mouseover', function(e){
			...
			var i = that.getRowIndex( value );
			if ( false === that._trigger("beforeSelect", null, [ {"item":that.options.data[i]},{"data":that.options.data[i]} ]) ) {
				return false;
			} 
			if (that.options.multiple) {
				that.oldText = that.uiCombo.textbox.val();// 20150121重新设置之前保存下text
				if ("" === value) {
					that.clear();
					that.select(value);
					that.hidePanel();
	...
	_renderItems: function( data ){
		var opts     = this.options,
			...
			selected = [];
		var itemStr = [];
		var formatterEvent = $.coral.toFunction(this.options.formatter);
		for(var i = 0; i < data.length; i++) {
			var row = data[i],
				v = row[opts.valueField],
				s = row[opts.textField],
				hidden = !!row.hidden==true?"hidden":"",
				fmt = s;
			if ( formatterEvent ) {
				fmt = formatterEvent.call( this.element, data[i] );
			}
			
			itemStr.push("<div class='coral-combobox-item "+hidden+" ' id='"+this.options.itemIdPrefix+"_"+i+"' value='"+v+"'>"+fmt+"</div>");
	...
	loadData: function(data, remainText) {
		...
		if (this.options.value) {
			selected = this.options.value.split(",");
		}
		// 已设值且不为空，则优先级别为最高
		values = this._getCurrentValues();
		if ("" !== values.toString()) {
			selected = values;
		}
		// 如果没有设置默认值，则值为空
		...
		if (opts.multiple) {
			this.setValues(selected, remainText);
		} else {
	...
	/*	var opts = this.options, 
		...
		if ( $.isArray( url ) ) {
			...
			loaderEvent.apply(this.element, [param, function(data) {
				...
				that._trigger("onLoad", null, [data]);
				that._loadedHandler();
			}, function() {
	    ...
	    if ( isUrl ){
			...
			loaderEvent.apply(this.element, [param, function(data) {
	    	 ...
	    	 that.loadData(data, remainText);
	    	 that._trigger( $.isFunction( opts.onLoad )?opts.onLoad:"onLoad", null, [{content:data}] );
	     }
	},
	/**
	 * 加载后执行缓存的方法
	...
	_loadedHandler: function() {
		...
		if (item_setValues) {
			this.setValues(item_setValues.values, item_setValues.remainText, item_setValues.triggerOnChange);
	...
	setValues: function( values, remainText, triggerOnChange ) {
		...
		this._super( valueArr, remainText, triggerOnChange );
	},
	/**
	...
	reset: function() {
		var opts = this.options;
		if (opts.multiple){
			this.setValues(opts.originalValue, false, false);
		} else {
