...
function eXcell_sub_row_form(cell) {
    this.base = eXcell_sub_row;
    this.base(cell);
    ...
    this.setValue = function(val) {
        if (val)
            this.grid.setUserData(this.cell.parentNode.idd, "__sub_row", val);
        this.cell._sub_row_type = "form";
        this._setState("plus.gif");
    }
    this.getSubForm = function() {
        if (!cell._sub_form)
            return null;
        return cell._sub_form;
    }
}
eXcell_sub_row_form.prototype = new eXcell_sub_row;
    ...
    if (td._sub_row_type == "form") {
		if (row._expanded && !show) {
            ...
            if (td._previous_content) {
                ...
                d.ctrl = td;
                if (td._sub_row_type)
                    that._sub_row_render[td._sub_row_type](that, d, td);
            ...
            if (!that._flow) {
                ...
                that.attachEvent("onClearAll", function() {
                    for (var i in this._flow) {
                        if (this._flow[i] && this._flow[i].parentNode)
                            this._flow[i].parentNode.removeChild(this._flow[i]);
                    }
                    ;
                    this._flow = [];
                });
        ...
        if (row._expanded && !show) {
            ...
            if (!that._flow) {
                ...
                that.attachEvent("onEditCell", function(a, b, c) {
                    if ((a !== 2) && this._flow[b] && this.cellType[c] != "ch" && this.cellType[c] != "ra")
                        this._expandMonolite.apply(this._flow[b].ctrl.firstChild, [0, false, true]);
                    return true;
                });
    ...
    "grid" : function(that, d, td, c) {
        ...
        td._sub_grid.attachEvent("onGridReconstructed", function() {
            that._detectHeight(d, td, td._sub_grid.objBox.scrollHeight + td._sub_grid.hdr.offsetHeight
                            + (this.ftr ? this.ftr.offsetHeight : 0));
            that._correctMonolite();
            this.setSizes();
            if (that.parentGrid)
                that.callEvent("onGridReconstructed", []);
        ...
        td._sub_grid.loadXML(c, function() {
            that._detectHeight(d, td, td._sub_grid.objBox.scrollHeight + td._sub_grid.hdr.offsetHeight
                            + (td._sub_grid.ftr ? td._sub_grid.ftr.offsetHeight : 0));
            td._sub_grid.objBox.style.overflow = "hidden";
            that._correctMonolite();
            td._sub_row_type = null;
            if (!that.callEvent("onSubGridLoaded", [td._sub_grid, td.parentNode.idd, td._cellIndex, c]))
                return;
            if (that._ahgr)
                that.setSizes();
        });
    },
    "form" : function(that, d, td) {
        var cont = document.createElement("div");
        document.body.appendChild(cont);
        td._sub_form = new dhtmlXForm(cont);
        that.callEvent("onBeforeSubFormLoadStruct", [td._sub_form, td.parentNode.idd]);
        if (!td._sub_form.c)
        	return;
        td._sub_form.parentGrid = that;
        td._sub_form.loadStruct(td._sub_form.c, "json");
        d.appendChild(cont);
        that._detectHeight(d, td);
        that._correctMonolite();
        td._sub_row_type = null;
        if (that._ahgr)
            that.setSizes();
        that.callEvent("onSubFormLoaded", [td._sub_form, td.parentNode.idd, td._cellIndex]);
    }
}
...
dhtmlXGridObject.prototype.addSubRow = function() {
    var first = (this.currentPage - 1) * this.pageSize || 0;
    if (this.getRowId(first) == '') {
        return;
    }
    this.addRow('', '', first);
    this.selectRow(first, true);
}
