...
window[CFG_actionName(subffix)] = function (ui) {
	...
	return jQuery.contextPath + "/zzcstj";
};
...
function _override_bsearch (ui) {
	...
	//};
    ui._init = function() {
        var dkxxData = $.loadJson($.contextPath + '/zzdyxx!getDkxx.json');
        var pzxx = $.loadJson($.contextPath + '/zzcpxxgl!getPlxxByPzxx.json');
        var pzxxData = [];
        $.each(pzxx,function(index,data){
            var pz = {};
            pz.PZ = data.text;
            pz.PZBH = data.text;
            pzxxData.push(pz);
        });
        var $dk = ui.getItemJQ("DKBH");
        var $pz = ui.getItemJQ("PZ");
        $dk.combogrid("reload",dkxxData);
        $pz.combogrid("reload",pzxxData);
    }
