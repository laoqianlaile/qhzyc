...
<%
	...
	if (StringUtil.isEmpty(systemName)) {
		systemName = "德令哈中药材种植加工流通追溯系统 V1.0";
	}
	//设置企业编码的session
...
<head>
	...
	<title>德令哈中药材种植加工流通追溯系统 V1.0</title>
	...
	<link href="<%=contextPath %>/cfg-resource/coral40/common/themes/bwt/css/css.css" rel="stylesheet"/>
	<link href="<%=contextPath %>/cfg-resource/coral40/common/themes/bwt/css/reset.css" rel="stylesheet"/>
...
<script type="text/javascript">
    ...
    $(function () {
        ...
        $.getJSON(contextPath + "/menu/menu!loadAppMenuForCoral40.json?systemId=" + systemId + "&P_timestamp=" + getTimestamp(), function (data) {
            ...
            $('.logo').append('<img src="<%=contextPath %>/cfg-resource/coral40/common/themes/bwt/images_bwt/logo4.png">');
                ...
                if(menuA.items.length==0){//菜单下面没有子菜单的走这里
                    if(menuA.code == 'sy'){//加载首页菜单样式
                        var sy ='<span class="menuList">'+
                            '<span class="menuList"><a href="javascript:void(0)" class="iconIndex" onclick="menuClick(\'工作台\',\'/cfg-resource/coral40/views/component/sdzycbwttz/sdzycbwttz.jsp\',this)"></a> </span></span>';
