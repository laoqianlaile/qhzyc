...
$.component("coral.combo", $.coral.control, {
	...
	_initCombo: function() {
		...
		if (this.options.hasDownArrow) {
			comboArrow = "<span class='coral-combo-arrow coral-icon-arrow icon icon-arrow-down2'></span>";
		} 
		// 删除图标
		if ( this.options.showClose && !this.options.required) {
			clearIcon = "<span class='coral-combo-close icon icon-close2'></span>";
		}
		...
		// 下拉面板
		this.uiCombo.panel = $( "<div class='coral-combo-panel'></div>" ).appendTo( "body" );
		...
		if ( this.options.enableFilter ) {
			$( "<div class='coral-combo-filter'><span class='coral-combo-filter-border'><input type='text' class='coral-combo-filterbox'></span>" +
				"<span class='coral-combo-search icon icon-search3'></span></div>" ).appendTo( this.uiCombo.panel );
		}
	...
	_setDefaultValue: function() {
		...
		if ( this.options.multiple ) {
			...
			this.options.originalValue = this.getValues();
		} else {
			this.setValues( [this.options.value] );
	...
	_hidePlaceholder: function () {
		...
		that.component().find( ".coral-textbox-placeholder-label" ).remove();
	},
	/**
	 *
	 */
	...
	_bindEvent: function() {
		...
		this._on({
			...
			"keydown.coral-combo-text" : function(e) {
				...
				var panel  = this.panel(),
					item = panel.find(".coral-combobox-item");
			...
			"blur.coral-combo-text" : function(e) {
				...
				this._showItems();
				if ( inputVal.toString() != this.previousValue ) {
			...
			"click.coral-combo-close" : function(e) {
				if ( this.component().hasClass("coral-state-focus") ) {
					suppressBlurs = true;
				}
				this.clear();
		...
		if (that.options.enableFilter) {
			var filterbox = this.uiCombo.panel.find( ".coral-combo-filterbox" ),
				searchBtn = this.uiCombo.panel.find( ".coral-combo-search" );
			filterbox.bind( {
				"focusout": function(e) {
					//suppressBlurs = false;
				},
				"keyup": function(e) {
					if (!that.options.enterFilter) return;
					that._filter(e, true);
				}
			} );
			searchBtn.bind( {
	...
	_filter: function (e, searchOnly) {
		...
		that.timer = setTimeout(function () {
			...
			if ( !searchOnly ) {
				...
				that.setValues(v.valarr,true,false);
			}
			// 赋值 end
	...
	uiClose : function() {
		return this.uiCombo.combo.find("span.coral-combo-close");
	},
	uiBorder : function () {
	...
	clear : function() {
		var $valuebox = this.uiCombo.combo.find("input.coral-combo-value"),
			$textbox = this.uiCombo.combo.find("input.coral-combo-text");
	...
	setText : function( text ) {
		...
		/*var i = 0,
			...
			valueArr = [],
			textArr = text.split(this.options.separator);
		for( i; i < textArr.length; i++ ){
			var tText = textArr[ i ];
			for( j; j < data.length; j++ ) {
				if ( data[ j ][ this.options.textField ] == tText ) {
					valueArr.push( data[ j ][ this.options.valueField ] );
					break;
				}
			}
		}
		if ( valueArr.length > 0 ) this.setValues( valueArr );*/
	},
	/**
	 * 返回数组格式的值
	 */
	 ...
	 */
	setValues : function(values, remainText, triggerOnChange ) {
		...
		if ( (eqTmp.length != values.length || values.length != oldValues.length) && 
				( !remainText ) && triggerOnChange) {
			if (this.options.multiple) {
				this._trigger("onChange", null, {
					value: values, newValue: values, 
					newText: this.getText(), text: this.getText(), 
					oldValue: oldValues, oldText: this.getOldText()});
			} else {
				// TODO: 此处values[0]，应该改为values.join( 分隔符 );
				this._trigger("onChange", null, {
					value: values[0], newValue: values[0], 
					newText: this.getText(), text: this.getText(), 
					oldValue: oldValues[0], oldText: this.getOldText()});
			}
		}
	},
	/**
