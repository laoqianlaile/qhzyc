package com.ces.component.qhpfyljydwjscxg.service;
...
@Component
public class QhpfyljydwjscxgService extends TraceShowModuleDefineDaoService<StringIDEntity, QhpfyljydwjscxgDao> {
    ...
    public Object saveAll(String tableId, String entityJson, String dTableId, String dEntitiesJson, Map<String, Object> paramMap) {//
        ...
        String sql = "select t.id,t.pch from t_qh_pfrcxx t where t.pch=?";
        String pch = dataMap.get("PCH");
        Map<String, Object> traceMap = DatabaseHandlerDao.getInstance().queryForMap(sql, new String[]{pch});
        ...
        //修改药材入库信息中的检验结果
        String updSql = "update t_qh_pfrcxx set jyjg=? where  pch=?";
    ...
    protected List<Map<String, String>> saveDetail(String tableName, String entitiesJson, Map<String, String> masterMap, Map<String, String> relateDateMap, Map<String, Object> paramMap, List<TestFileEntity> dataList,
        ...
        for (int i = 0, len = entities.size(); i < len; i++) {
            ...
            if (imgBase64 != null) {
                ...
                if (array.length >= 2 || imageType.length >= 2) {//判断在修改信息时是否有已保存的图片(是)
                    imgBase64 = array[1];
                    ...
                    String s = dataMap.get("ID").substring(0, 6);
                    if (s.equals("UNSAVE")) {
                        dataMap.remove("ID");
                    ...
                    testfileEntity.setJyry((masterMap.get("JYRY")));
