...
<script>
    ...
    $(function () {
        ...
        var $grid = $("#gridDemo${idSuffix}"),
                _colModel = [
                    ...
                    {name: "ZZDYMC", sortable: true, width: 200, fixed: true},
                    {name: "GGFS", sortable: true, width: 200, fixed: true},
                    {name: "SYLX", sortable: true, width: 200, fixed: true},
                    {name: "SYDJ", sortable: true, width: 200, fixed: true},
                    {
                        name: "options",
                        width: 200,
                        formatter:"toolbar",
                ...
                _colNames = ["id", "单元名称","灌溉方式", "水源类型","水源等级","操作"],
                _setting = {
                    width: "auto",
                    height: "400",
                    colModel: _colModel,
                    colNames: _colNames,
                    rowNum:10,
                    url: $.contextPath + baseUrl + "/queryGgPage?qybm=" + qybm + "&qybh=<%=qybh%>&dkbh=<%=dkbh%>&dybh=<%=dybh%>"
                };;
        $grid.grid(_setting);
    })
    function toolbarClick1(e,ui){
        ...
        var idList;
        $.ajax({
            url:$.contextPath + baseUrl + "/queryGgIds?qybm=" + qybm + "&qybh=<%=qybh%>&dkbh=<%=dkbh%>&dybh=<%=dybh%>",
            type:"post",
            async:false,
    ...
    gridClick = function (e, ui) {
        ...
        if(ui.id=="again"){//再做一次
            $.ajax({
                type:"post",
                url:$.contextPath + baseUrl + "/doAgainGg",
                dataType:"json",
