...
<div id="max${idSuffix}" class="fill">
    <div class="toolbarsnav clearfix">
        <div class="coral-toolbar-item-component coral-toolbar-html ctrl-toolbar-element" group="upper left" data-frozen="true" style="right: auto; left: 0px;">
            <h1>
                <span class="homeSpan" style="padding-left: 30px;">- 饮片加工量统计
                </span>
            </h1>
        </div>
    </div>
    <ces:form id="form${idSuffix}" name="form">
        <div class="fillwidth colspan3 clearfix ">
            ...
            <div class="app-inputdiv4">
                <label class="app-input-label">商品名称：</label>
                <input class="coralui-textbox" id="cpmc${idSuffix}" name="CPMC"/>
            </div>
...
<script type="text/javascript">
    ...
    $.extend($.ns("namespaceId${idSuffix}"), {
        toolbarClick : function ( e , ui ){
            ...
            if(!$("#form${idSuffix}").form("valid"))return;
            if( ui.id == 'reset' ){
                var $form = $("#form${idSuffix}");
                $form.form("reset");
            ...
            if( ui.id == 'search' ){
                ...
                var jsonData = $.loadJson($.contextPath + "/sdzycjjgypjgltj!searchj.json?kssj="+$formData.KSSJ+"&jssj="+$formData.JSSJ+"&rqlx="+$formData.RQLX+"&cpmc="+$formData.CPMC);
                if(isEmpty(jsonData.series)){
                    CFG_message("无数据显示","warning");
                }else{
                    initCharts(jsonData);
                }
    ...
    $(function() {
        var configInfo = CFG_initConfigInfo({
            /** 页面名称 */
            'page' : 'sdzycjjgypjgltj.jsp',//入库量趋势分析页面
            /** 页面中的最大元素 */
