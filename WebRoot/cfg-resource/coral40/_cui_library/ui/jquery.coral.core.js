...
$.extend( $.coral, {
	...
	adjusted : function(element){
		...
		$element.height( Math.max( 0, maxHeight - $element.innerHeight() + $element.height() ) )
				.css( "overflow", "auto" ).addClass("overflow-hidden");
	},
	/**
	 * 处理 overflow
	 **/
	beforeDoOverflow : function(element){
		...
		//$(".coral-panel-body").addClass("overflow-hidden");
	},
	/**
	 * 处理 overflow
	 **/
	 ...
	 **/
	refreshAllComponent : function(element){
		$.coral.beforeDoOverflow();

		if(typeof(element)=="undefined"){
			element = "body";
		}
		$(".ctrl-init-layout:visible, .ctrl-init-accordion:visible, .ctrl-init-tabs:visible, .ctrl-init-grid:visible," +
			" .ctrl-init-form:visible, .coral-adjusted:visible, .ctrl-init-toolbar", $(element)).each(function() {
				/*if ( $(this).closest(".ctrl-fit-element")[0] == $(element)[0] ) {
					console.log("ctrl-fit-element");
				} else {
					console.log("not fit");
				}*/
				/*if ( ($(this).closest(".ctrl-fit-element")[0] == $(element)[0] ||
						$(this).closest(".coral-adjusted")[0] == $(element)[0]) && 
						$(this).closest(".ctrl-fit-element")[0] != $(this).closest(".coral-adjusted")[0] ) {
						console.log("ctrl-fit-element");
					} else {
						console.log("not fit");
					}*/
				...
				if ( $(this).hasClass( "coral-adjusted" ) ){
					$.coral.adjusted(this);
				}
		});
		$.coral.doOverflow();
