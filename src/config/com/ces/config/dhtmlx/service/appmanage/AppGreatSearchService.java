...
@Component
public class AppGreatSearchService extends ConfigDefineDaoService<AppGreatSearch, AppGreatSearchDao> {
     ...
     */
    @Autowired
    @Qualifier("appGreatSearchDao")
    @Override
    protected void setDaoUnBinding(AppGreatSearchDao dao) {
     ...
     * @return List<AppGreatSearch> 返回类型
     */
     ...
     * @throws
     */
    public List<AppGreatSearch> findDefaultList(String tableId, String componentVersionId, String menuId) {
        List<AppGreatSearch> list = new ArrayList<AppGreatSearch>();
        ...
        for (Object[] oArr : rlt) {
        	...
        	AppGreatSearch item = new AppGreatSearch();
     ...
     * @throws
     */
    public List<AppGreatSearch> findDefineList(String tableId, String componentVersionId, String menuId) {
        List<AppGreatSearch> list = new ArrayList<AppGreatSearch>();
        List<Object[]> rlt = getDefineColumn(tableId, componentVersionId, menuId);
        for (Object[] oArr : rlt) {
            // t_as.id,  t_cd.id as column_id, t_cd.show_name, t_cd.column_name,t_as.filter_type, t_cd.data_type, t_cd.code_type_code, t_cd.input_type
        	AppGreatSearch item = new AppGreatSearch();
            ...
            item.setDataType(StringUtil.null2empty(oArr[4]));
            item.setDataTypeExtend(StringUtil.null2empty(oArr[5]));
            item.setCodeTypeCode(StringUtil.null2empty(oArr[6]));
            item.setInputType(StringUtil.null2empty(oArr[7]));
            list.add(item);
        }
        return list;
    }
     ...
     */
    @Transactional
    public void save(AppGreatSearch master, String rowsValue) throws FatalException {
        ...
        AppGreatSearch entity = null;
        List<AppGreatSearch> list = Lists.newArrayList();
        for (int i = 0; i <rowsArr.length; i++) {
            String columnId = rowsArr[i];
            entity = new AppGreatSearch();
     ...
     */
    @Transactional
    public void clear(AppGreatSearch entity) throws FatalException {
