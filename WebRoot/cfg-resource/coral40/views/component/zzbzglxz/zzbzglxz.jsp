...
<div id="max${idSuffix}" class="fill"> 
    <div class="fill">
        <div class="toolbarsnav clearfix">
            ...
            <div class='homeSpan' style="margin-top: -23px;"><div><div style='margin-left:20px;width: 150px;' id="nva${idSuffix}"> - 包装管理 - 新增 </div></div></div>
        </div>
        <form id="bzForm${idSuffix}" action="${basePath}"
            ...
            <div class="fillwidth colspan3 clearfix">
                ...
                <%--<div class="app-inputdiv4" style="height:32px;display: block">--%>
                ...
                <div class="app-inputdiv4">
                    ...
                    <input id="cpgg${idSuffix}" data-options="readonly:true,texticons:'公斤'" name="BZGG" class="coralui-textbox"/>
                </div>
                <%--<div class="app-inputdiv4" style="height:32px;display: none">--%>
        ...
        <div class="toolbarsnav clearfix">
            <ces:toolbar id="toolbarId1${idSuffix}" onClick="$.ns('namespaceId${idSuffix}').toolbarClick1"
                         data="['->',{'label': '读码', 'id':'readCode', 'disabled': 'false','type': 'button'},'','','','','','']">
            </ces:toolbar>
        </div>
        <ces:grid id="gridId${idSuffix}" shrinkToFit="true" forceFit="true" fitStyle="fill" datatype="local"
            ...
            <ces:gridCols>
              ...
              <!--  <ces:gridCol name="CSPCH" editable="false"  width="100">CSPCH</ces:gridCol>-->
                <ces:gridCol name="BZPCZSM" editable="false" hidden="true" formatoptions=""
                ...
                <ces:gridCol name="KCZL" editable="false"  width="100">库存重量</ces:gridCol>
                <ces:gridCol name="JGQZL" editable="true" formatter="text" formatoptions="validType:'float',onChange:$.ns('namespaceId${idSuffix}').judge,required: true"
                             width="100">加工前重量</ces:gridCol>
                <ces:gridCol name="JGHZL" editable="true" formatter="text" formatoptions="validType:'float',onChange:$.ns('namespaceId${idSuffix}').textEvaluate,required: true"
                ...
                <ces:gridCol name="SHZL" editable="true" formatter="text" formatoptions="validType:'float',required: true,readonly:true"
                             width="100"  >损耗重量</ces:gridCol>
                <ces:gridCol name="BZJS" editable="true" formatter="text" formatoptions="validType:'naturalnumber'"
                             width="100">包装件数</ces:gridCol>
            </ces:gridCols>
        </ces:grid>
    </div>
</div>
...
<script type="text/javascript">
    ...
    $.extend($.ns("namespaceId${idSuffix}"), {
        cpmcClick: function () {
            CFG_clickButtonOrTreeNode($('#max${idSuffix}').data('configInfo'), "cpmc", "产品名称弹出框", 2, $.ns("namespaceId${idSuffix}"));
        },
       ...
       textEvaluate:function (){
            ...
            for(var i in gridDataIDs){
                ...
                //在计算损耗重量前判断加工后重量非负
                var shzl = gridData.JGQZL-gridData.JGHZL;
                //判断损耗重量是否大于0
                if(shzl>=0&&gridData.JGHZL>=0){
                    $grid.grid("setCell",gridDataIDs[i],'SHZL',shzl);
                }else{
                    //不是，则加工后重量置为加工前重量，意为0损耗
                    $grid.grid("setCell",gridDataIDs[i],'JGHZL',gridData.JGQZL);
                    $grid.grid("setCell",gridDataIDs[i],'SHZL',0);
                }
          		}
       },
        //加工前重量非零且要小于库存重量
        judge:function(){
            ...
            for(var  i in gridDataIDs){
                var gridData = $grid.grid("getRowData",gridDataIDs[i]);
                //判断加工前重量是否大于库存重量，是 ，改为库存重量
                var jgqzl = (gridData.JGQZL-gridData.KCZL>=0?gridData.KCZL:gridData.JGQZL);
                $grid.grid("setCell",gridDataIDs[i],'JGQZL',jgqzl);
            }
        }
    });
    $(function () {
        var configInfo = CFG_initConfigInfo({
            ...
            'bodyOnLoad': function (configInfo) {
                ...
                /****************初始化数据 begin****************************/
                //初始化日期框
                $("#bzsj${idSuffix}").datepicker("option","dateFormat","yyyy-MM-dd HH:mm:ss");
                $("#bzsj${idSuffix}").datepicker("option","srcDateFormat","yyyy-MM-dd HH:mm:ss");
    ...
    $("#bzForm${idSuffix}").submit(function () {
        ...
        formData.MDBH=$('#mdbh${idSuffix}').combobox('getText');
        var gridData = $grid.grid("getRowData");
        var bool = false;
        $.each(gridData,function(e,data){
            if(parseFloat(data.JGQZL) > parseFloat(data.KCZL)){
                bool = true;
            }
        });
        if(bool){
            CFG_message("加工前重量大于库存重量！", "error");
            return false;
        }
        var postData = {
    ...
    function showDialog() {
        ...
        jqUC.dialog({
            ...
            onCreate: function () {
                ...
                var jq = $("<input id=\"UNTREAD_OPINION_READCODE\" name=\"opinion\" onkeydown=\"javascript:if (event.keyCode==13) subData();\"/>").appendTo(jqDiv);
        ...
        if(dataJson == false || dataJson == "false"){
            ...
            for(var i in gridDataIDs){
                ...
                if(gridData.PZBH == dataJson.PZBH){
                    ...
                    $grid.grid("setCell",gridDataIDs[i],'KCZL',dataJson.KCZL);
                    bool = 1;
                }
            }
            if(bool == 0){
