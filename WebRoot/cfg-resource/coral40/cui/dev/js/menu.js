    ...
    $.component("coral.basemenu", {
        ...
        setActiveMenu : function(menuNames){
        	...
        	$.each(menuNames,function(){
        		...
        		aEle.parent().addClass("coral-menu-item-current");
        	});
        },
        setCurrenMenu : function(menuNames){
        	...
        	this.element.find(">li>a").each(function() {
				...
				elem.parent().addClass("coral-menu-item-current");
				$(this).parent().siblings().removeClass("coral-menu-item-current");
			});
        }
    });
})(jQuery);
...
$.component("coral.navigationmenu", $.coral.basemenu,{
    ...
    _create: function() {
        ...
        this.links = this.element.find('.coral-menu-item-link:not(.coral-state-disabled)');
        this._on({
			...
			// 第一级的处理
			"click .coral-menu-item-link": function( event ) {
				event.preventDefault();
				var target = $( event.target ).closest( ".coral-menu-item-link" ),
			...
			"mouseenter .coral-menu-item-link": function( event ) {
				event.preventDefault();
				var target = $( event.target ).closest( ".coral-menu-item-link" );
			...
			"mouseleave .coral-menu-item-link": function( event ) {
				event.preventDefault();
				var target = $( event.target ).closest( ".coral-menu-item-link" );
				var parent = $( event.target ).closest( ".coral-menu-parent" );
				if ( parent.not( ".coral-state-disabled" ).length ) {
    ...
    _updateTitle: function(){
    	this.element.find("li>a").each(function() {
			var elem = $( this ),
				dn = elem.find(".coral-menu-item-text").text();
    ...
    _render: function() {
        ...
        this.element.find('li').each(function() {
	        ...
	        menuitemLink.addClass('coral-menu-item-link coral-state-default coral-corner-all');
	        menuitemLink.contents().filter(function(i, item){
	        	if (item.tagName) return false;
	        	return true;
	        }).wrap("<span class='coral-menu-item-text'/>");
	        ...
	        listItem.addClass('coral-menu-item coral-component coral-corner-all')
	        	.attr("data-level", level).addClass("level"+level);
	        /*if(icon) {
	        	menuitemLink.prepend('<span class="coral-menu-item-icon coral-icon ' + icon + '"></span>');
	        }*/
...
/*$.component("coral.panelmenu", $.coral.basemenu,{
    ...
    _create: function() {
        ...
        this._on({
			...
			"click .coral-menu-item": function( event ) {
				var target = $( event.target );
				target = $( target ).closest( ".coral-menu-item-link" )
				if ( target.not( ".coral-state-disabled" ).length ) {
					if ( target.parent().hasClass( "coral-menu-parent") ){
    ...
    _render: function() {
        ...
        this.element.find('li').each(function() {
	        ...
	        menuitemLink.addClass('coral-menu-item-link coral-state-default coral-corner-all').contents().wrap('<span class="coral-menu-item-text" />');
        ...
        _create: function() {
            ...
            this.element.children('li').each(function() {
                ...
                if(listItem.children('h3').length > 0) {
                    ...
                    menuitemLink.addClass('coral-menu-item-link coral-corner-all').contents().wrap('<span class="coral-menu-item-text" />');
            ...
            this.menuitemLinks = this.element.find('.coral-menu-item-link:not(.coral-state-disabled)');
    ...
    $.component("coral.breadcrumb", {
        ...
        _create: function() {
            ...
            this.element.children('li').each(function(index) {
                ...
                if(index > 0)
                    listItem.before('<li class="coral-breadcrumb-chevron coral-menu-icon cui-icon-arrowDown1"></li>');
                else
...
(function ($) {
    ...
    $.component("coral.tieredmenu", $.coral.basemenu, {
        ...
        _create: function() {
            ...
            this.links = this.element.find('.coral-menu-item-link');
        ...
        _render: function() {
            ...
            this.element.find('li').each(function() {
                    ...
                    if(listItem.children('ul').length > 0) {
                        ...
                        menuitemLink.prepend('<span class="coral-menu-icon cui-icon-arrowDown1"></span>');
                    }
        ...
        _bindMenuItemClick: function ($this, containRootLink) {
        	...
        	if (typeof containRootLink === "boolean" && !containRootLink) {
        		rootLinks = this.element.find('> .coral-menu-item > .coral-menu-item-link');
        	}
        ...
        _bindItemEvents: function() {
            ...
            this.links.on('mouseenter.coral-menu',function() {
                ...
                var link = $(this),
	                ...
	                activeSibling = menuitem.siblings('.coral-menu-item-active');
                
                if(activeSibling.length === 1) {
                    $this._deactivate(activeSibling);
                }
                if(autoDisplay||$this.active) {
                    if (menuitem.hasClass('coral-menu-item-active')) {
            ...
            if(this.options.autoDisplay === false) {
                this.rootLinks = this.element.find('> .coral-menu-item > .coral-menu-item-link');
        ...
        _addByParentId: function(pid, data) {
        	var that = this,
        		$pNode = this.element.find(".coral-menu-item").filter("[data-id$='" + pid + "']");
        ...
        _renderAddHTML: function($pNode) {
        	var $this = this,
        		$links = this.element.find('li').not(".coral-menu-item");
        	...
        	if( $pNode && !$pNode.children('ul').hasClass("coral-menu-list")) {
        		...
        		$pNode.children("a").prepend('<span class="coral-menu-icon cui-icon-arrowDown1"></span>');
            }
            ...
            return $links.find('.coral-menu-item-link');
        },
        /**
         * 给根菜单绑定事件，用以显示子菜单
        ...
        _bindAddItemEvents: function($links) {
        	...
        	$links.on('mouseenter',function() {
                ...
                var activeSibling = menuitem.siblings('.coral-menu-item-active');
                if(activeSibling.length === 1) {
                    $this._deactivate(activeSibling);
                }
		...
		_removeById: function (id) {
			var node = this.element.find(".coral-menu-item").filter("[data-id$='" + id + "']");
			if (!node.length) {
				return ;
			}
			// 如果父元素没有子元素，则将父元素也删除
		...
		_removeByIndex: function ( index ) {
			...
			var $removeNode = this.element.children("li.coral-menu-item:eq("+idx+")");
		...
		_updateById: function ( id, label ) {
			var nodeUpdate = this.element.find(".coral-menu-item").filter("[data-id$='" + id + "']");
			
			if ( !nodeUpdate.length ) {
				return ;
			}
			
			nodeUpdate.children(".coral-menu-item-link").children( ".coral-menu-item-text" ).html( label );	
		},
		/**
		 * 根据根菜单index修改项的文本
		 * @param index { number } :根菜单索引
		...
		_updateByIndex: function ( index, label ) {
			...
			var nodeUpdate = this.element.children("li.coral-menu-item:eq("+idx+")");
        ...
        _bindDocumentHandler: function() {
            ...
            $(document.body).on('click.coral-menu', function(e) {
                ...
                $this.element.find('li.coral-menu-item-active').each(function() {
         ...
         */
        enable: function() {
        	var node = this.element.find(".coral-menu-item");
			
			if ( !node.length ) {
				return ;
			}
			
			node.children(".coral-menu-item-link").toggleClass( this.componentFullName + "-disabled coral-state-disabled", false ).attr( "aria-disabled", false );
        },
        /**
	     * 根据id或者根菜单索引，启用
	     * @param key { string,number } : id，根菜单索引
        ...
        _enableItemByIndex: function(index) {
			...
			var node = this.element.children("li.coral-menu-item:eq("+idx+")");
        ...
        disable: function() {
			...
			node.children(".coral-menu-item-link").toggleClass( this.componentFullName + "-disabled coral-state-disabled", true ).attr( "aria-disabled", true );
        },
        /**
	     * 根据id或者根菜单索引，禁用
	     * @param key { string,number } : id，根菜单索引
        ...
        _deactivate: function(menuitem, animate) {
            ...
            menuitem.children('a.coral-menu-item-link').removeClass('coral-state-hover');
            menuitem.removeClass('coral-menu-item-active');
        ...
        _reactivate: function(menuitem) {
            ...
            var submenu = menuitem.children('ul.coral-menu-child'),
            activeChilditem = submenu.children('li.coral-menu-item-active:first'),
            _self = this;
        ...
        _highlight: function(menuitem) {
            this.activeitem = menuitem;
            menuitem.children('a.coral-menu-item-link').addClass('coral-state-hover');
            menuitem.addClass('coral-menu-item-active');
        },
...
(function ($) {
    ...
    $.component("coral.slidemenu", $.coral.basemenu, {
        ...
        _create: function() {
            ...
            this.links = this.element.find('a.coral-menu-item-link:not(.coral-state-disabled)');
...
(function ($) {
    ...
    $.component("coral.contextmenu", $.coral.tieredmenu, {
        ...
        _hide: function() {
            ...
            //hide submenus
            this.element.parent().find('li.coral-menu-item-active').each(function() {
                $this._deactivate($(this), true);
            });
