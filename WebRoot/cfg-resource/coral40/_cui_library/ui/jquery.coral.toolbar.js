			...
			if ( !this.isLoaded ) {
				...
				if ( null == index || index == innerLength ) {
					...
					for (var k in items) {
						...
						var coralType = $group.find(".ctrl-toolbar-element:eq(" + index + ")").attr("component-role");
						$group.find(".ctrl-toolbar-element:eq(" + index + ")")[coralType]("component").before(items[k].$el);
					}
				}
			}
		},
