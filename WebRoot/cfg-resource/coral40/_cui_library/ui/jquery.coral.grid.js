...
$.component( "coral.grid",{
	version: "4.0.1",
	castProperties : ["rowEditButtons","data", "initData", "rowList", "pagerTemplate"],
	options: {
	...
	_destroy: function(){
        ...
        this.component().replcaeWith(this.element).append(destroyPag); 
	},
	/***
	 * 第一步：
	 * 初始化生成统一的模型数据
	...
	_setUpEvent: function(){
		...
		$(grid.rowsView).scroll(function(e){
			...
			if (b.length > 0) {
				b.position({
					my: "left-"+(b.outerWidth()/2)+" top-2",
					at: "right-"+($(editRow).outerWidth()/2)+" top+"+($(editRow).outerHeight()),
					collision: "fit",
					of: $(editRow)
				});
			}
					...
					$(that.rows).each(function(i) {
						if (i>0) {
							if(!$(this).hasClass("coral-subgrid") && !$(this).hasClass("jqgroup") && !$(this).hasClass('coral-state-disabled')){
								...
								if(froz) {
									$("#jqg_"+$.grid.coralID(that.options.id)+"_"+$.grid.coralID(that.options.id), that.grid.fbDiv )[that.options.useProp ? 'prop': 'attr']("checked",true);
									$("#"+$.grid.coralID(that.options.id), that.grid.fbDiv).addClass("coral-state-highlight");
								}
							}
						}
					});
					...
					$(that.rows).each(function(i) {
						if(i>0) {
							if(!$(this).hasClass("coral-subgrid") && !$(this).hasClass('coral-state-disabled')){
								...
								emp.push(this.id);
								if(froz) {
									$("#jqg_"+$.grid.coralID(that.options.id)+"_"+$.grid.coralID(that.options.id), that.grid.fbDiv )[that.options.useProp ? 'prop': 'attr']("checked",false);
									$("#"+$.grid.coralID(that.options.id), that.grid.fbDiv).removeClass("coral-state-highlight");
								}
							}
						}
					});
		...
		if(pos>=0) {
			...
			if (ind){
				...
				if(nData !== "" || forceupd === true) {
					v = that._formatter(rowid, nData, pos,ind,'edit');
	...
	constructTr: function(id, hide, altClass, rd, cur) {
		var tabindex = '-1', restAttr = '', attrName, style = hide ? 'display:none;' : '',
			rowAttrObj,
			classes = 'coral-component-content jqgrow coral-row-' + this.options.direction + altClass;
...
*/			if (that.options.viewrecords){
	...
	_addMulti: function(rowid,pos,irow){
		var that = this,
			isPicgrid = !!(that.options.model == "card"),
			v = "<input role=\"checkbox\" type=\"checkbox\""+" id=\"jqg_"+that.options.id+"_"+rowid+"\" class=\"cbox\" name=\"jqg_"+that.options.id+"_"+rowid+"\"/>",
...
//		}
	...
	_addJSONData: function(data,t, rcnt, more, adjust) {
		...
		var rn = parseInt(that.options.rowNum,10),br=that.options.scroll?$.grid.randId():1, altr;
		...
		while (i<len) {
			...
			if( gi ){//如果是多选
				if(that.options.singleselect){
					rowData.push( that._addSingle(idr,ni,i) );
					buttonsData.push( that._addSingle(idr,ni,i) );
				}else{
					rowData.push( that._addMulti(idr,ni,i) );
					buttonsData.push( that._addMulti(idr,ni,i) );
				}
			}
			if( si ) {
			...
			if(that.options.model == "card"){
				...
				rowData[iStartTrTag] = that.constructTr(idr, hiderow, cn1, rd, cur);
				rowData.push( "</tr>" );
			}
