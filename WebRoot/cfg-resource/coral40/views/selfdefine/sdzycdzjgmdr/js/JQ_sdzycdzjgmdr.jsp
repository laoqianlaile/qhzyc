...
window[CFG_actionName(subffix)] = function (ui) {
    ...
    return jQuery.contextPath + "/sdzycdzjgmdr";
};
...
function _override_grid (ui) {
    ...
    //	return setting;
    //};
    ui.addOutputValue("setDzjgmPid_value",function ( res ) {
        var id = ui.getSelectedRowId();
        return {status: true};
    })
    ui.clickCoflowTrack = function (){
        var rowData = ui.uiGrid.grid("getRowData",ui.getSelectedRowId());
        window.open($.contextPath +"/sdzycdzjgmdr!downLoad?bzpch="+rowData.WJMC,"_blank");
    }
...
jQuery.extend(jQuery.ns("ns" + subffix), {
    viewImage : function (fileInput) {
            ...
            if(!(f.size<20971520 && (ImgSuffix=='txt'))){
                CFG_message("文件必须小于20M，且格式必须为.txt！", "error");
